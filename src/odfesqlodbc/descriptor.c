...
void TI_Destructor(TABLE_INFO **ti, int count) {
    ...
    MYLOG(DRV_TRACE, "entering count=%d\n", count);
    if (ti) {
        for (i = 0; i < count; i++) {
            if (ti[i]) {
                COL_INFO *coli = ti[i]->col_info;
                if (coli) {
                    MYLOG(DRV_ALL, "!!!refcnt %p:%d -> %d\n", coli, coli->refcnt,
                          coli->refcnt - 1);
                    coli->refcnt--;
                    if (coli->refcnt <= 0
...
static void ARDFields_free(ARDFields *self) {
    MYLOG(DRV_TRACE, "entering %p bookmark=%p\n", self, self->bookmark);
    if (self->bookmark) {
        free(self->bookmark);
        self->bookmark = NULL;
    }
...
char CC_add_descriptor(ConnectionClass *self, DescriptorClass *desc) {
    ...
    MYLOG(DRV_TRACE, "entering self=%p, desc=%p\n", self, desc);
...
RETCODE SQL_API ESAPI_CopyDesc(SQLHDESC SourceDescHandle,
    ...
    switch (srchd->desc_type) {
        case SQL_ATTR_APP_ROW_DESC:
            MYLOG(DRV_DEBUG, "src=%p target=%p type=%d", src, target,
                  srchd->desc_type);
            if (!targethd->type_defined) {
            ...
            ard_tgt = &(target->ardf);
            MYPRINTF(DRV_DEBUG, " target=%p", ard_tgt);
            ARDFields_copy(ard_src, ard_tgt);
            MYPRINTF(DRV_DEBUG, " offset_ptr=%p\n", ard_tgt->row_offset_ptr);
            break;
        case SQL_ATTR_APP_PARAM_DESC:
            if (!targethd->type_defined) {
                targethd->desc_type = srchd->desc_type;
        ...
        case SQL_ATTR_IMP_PARAM_DESC:
            ...
            break;
        default:
            MYLOG(DRV_DEBUG, "invalid descriptor handle type=%d\n",
                  srchd->desc_type);
...
RETCODE SQL_API ESAPI_DescError(SQLHDESC hdesc, SQLSMALLINT RecNumber,
    ...
    MYLOG(DRV_TRACE, "entering RecN=%hd\n", RecNumber);
