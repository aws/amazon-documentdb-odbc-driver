...
RETCODE SQL_API ESAPI_BindCol(HSTMT hstmt, SQLUSMALLINT icol,
    ...
    MYLOG(DRV_DEBUG, "**** : stmt = %p, icol = %d\n", stmt, icol);
    MYLOG(DRV_DEBUG, "**** : fCType=%d rgb=%p valusMax=" FORMAT_LEN " pcb=%p\n",
...
#pragma clang diagnostic pop
    ...
    if (rgbValue == NULL) {
        ...
        MYLOG(DRV_DEBUG, "       bound buffer[%d] = %p\n", icol,
...
void reset_a_parameter_binding(APDFields *self, int ipar) {
    MYLOG(DRV_TRACE, "entering ... self=%p, parameters_allocated=%d, ipar=%d\n",
...
void reset_a_column_binding(ARDFields *self, int icol) {
    ...
    MYLOG(DRV_TRACE, "entering ... self=%p, bindings_allocated=%d, icol=%d\n",
...
void ARD_unbind_cols(ARDFields *self, BOOL freeall) {
    ...
    MYLOG(DRV_ALL, "freeall=%d allocated=%d bindings=%p\n", freeall,
...
void GDATA_unbind_cols(GetDataInfo *self, BOOL freeall) {
    ...
    MYLOG(DRV_ALL, "freeall=%d allocated=%d gdata=%p\n", freeall,
...
void extend_getdata_info(GetDataInfo *self, SQLSMALLINT num_columns,
    ...
    if (self->allocated < num_columns) {
        ...
        if (!new_gdata) {
            MYLOG(DRV_DEBUG, "unable to create %d new gdata from %d old gdata\n",
                  num_columns, self->allocated);
...
void extend_putdata_info(PutDataInfo *self, SQLSMALLINT num_params,
    ...
    if (self->allocated < num_params) {
        if (self->allocated <= 0 && self->pdata) {
            MYLOG(DRV_DEBUG, "??? pdata is not null while allocated == 0\n");
            self->pdata = NULL;
        }
        new_pdata = (PutDataClass *)realloc(self->pdata,
                                            sizeof(PutDataClass) * num_params);
        if (!new_pdata) {
            MYLOG(DRV_DEBUG, "unable to create %d new pdata from %d old pdata\n",
                  num_params, self->allocated);
