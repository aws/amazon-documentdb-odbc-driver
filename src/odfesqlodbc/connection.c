...
RETCODE SQL_API API_Connect(HDBC hdbc, const SQLCHAR *szDSN,
    ...
    MYLOG(DRV_TRACE, "entering..cbDSN=%hi.\n", cbDSN);
...
RETCODE SQL_API API_Disconnect(HDBC hdbc) {
    ...
    MYLOG(DRV_DEBUG, "about to CC_cleanup\n");

    /* Close the connection and free statements */
    ret = CC_cleanup(conn, FALSE);

    MYLOG(DRV_DEBUG, "done CC_cleanup\n");
...
RETCODE SQL_API ESAPI_FreeConnect(HDBC hdbc) {
    ...
    MYLOG(DRV_TRACE, "entering...hdbc=%p\n", hdbc);
...
char CC_Destructor(ConnectionClass *self) {
    ...
    MYLOG(DRV_DEBUG, "after CC_Cleanup\n");
    ...
    MYLOG(DRV_DEBUG, "after free statement holders\n");
...
CC_cleanup(ConnectionClass *self, BOOL keepCommunication) {
    ...
    MYLOG(DRV_DEBUG, "after LIB_disconnect\n");
...
#define NULLCHECK(a) (a ? a : "(NULL)")
    ...
    if (self) {
        ...
        MYLOG(DRV_ERROR, "            esconn=%p, stmts=%p, lobj_type=%d\n",
              self->conn, self->stmts, self->lobj_type);
    } else {
        MYLOG(DRV_ERROR, "INVALID CONNECTION HANDLE ERROR: func=%s, desc='%s'\n",
              func, desc);
    }
}
