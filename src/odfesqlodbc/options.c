...
static RETCODE set_statement_option(ConnectionClass *conn, StatementClass *stmt,
    ...
    switch (fOption) {
        ...
        case SQL_CONCURRENCY:
             ...
             * read-only
             */
            MYLOG(DRV_DEBUG, "SQL_CONCURRENCY = " FORMAT_LEN " ", vParam);
        ...
        case SQL_CURSOR_TYPE:
             ...
             */
            MYLOG(DRV_DEBUG, "SQL_CURSOR_TYPE = " FORMAT_LEN " ", vParam);
        ...
        case SQL_KEYSET_SIZE: /* ignored, but saved and returned	*/
            MYLOG(DRV_DEBUG, "SQL_KEYSET_SIZE, vParam = " FORMAT_LEN "\n",
                  vParam);
        ...
        case SQL_MAX_LENGTH: /* ignored, but saved */
            MYLOG(DRV_DEBUG, "SQL_MAX_LENGTH, vParam = " FORMAT_LEN "\n",
                  vParam);
            if (conn)
        ...
        case SQL_MAX_ROWS: /* ignored, but saved */
            MYLOG(DRV_DEBUG, "SQL_MAX_ROWS, vParam = " FORMAT_LEN "\n", vParam);
            if (conn)
        ...
        case SQL_NOSCAN: /* ignored */
            MYLOG(DRV_DEBUG, "SQL_NOSCAN, vParam = " FORMAT_LEN "\n", vParam);
            break;

        case SQL_QUERY_TIMEOUT: /* ignored */
            MYLOG(DRV_DEBUG, "SQL_QUERY_TIMEOUT, vParam = " FORMAT_LEN "\n",
                  vParam);
            if (conn)
        ...
        case SQL_RETRIEVE_DATA:
            MYLOG(DRV_DEBUG, "SQL_RETRIEVE_DATA, vParam = " FORMAT_LEN "\n",
                  vParam);
            if (conn)
        ...
        case SQL_ROWSET_SIZE:
            MYLOG(DRV_DEBUG, "SQL_ROWSET_SIZE, vParam = " FORMAT_LEN "\n",
                  vParam);
...
RETCODE SQL_API ESAPI_SetConnectOption(HDBC hdbc, SQLUSMALLINT fOption,
    ...
    MYLOG(DRV_TRACE, "entering fOption = %d vParam = " FORMAT_LEN "\n", fOption,
          vParam);
    if (!conn) {
...
RETCODE SQL_API ESAPI_GetConnectOption(HDBC hdbc, SQLUSMALLINT fOption,
    ...
    switch (fOption) {
        ...
        case 1209:
            ...
            MYLOG(DRV_DEBUG, "CONNECTION_DEAD status=%d", conn->status);
        ...
        case SQL_ATTR_ANSI_APP:
            *((SQLUINTEGER *)pvParam) = CC_is_in_ansi_app(conn);
            MYLOG(DRV_DEBUG, "ANSI_APP val=" FORMAT_UINTEGER "\n",
                  *((SQLUINTEGER *)pvParam));
            break;
    ...
    switch (fOption) {
        ...
        case SQL_MAX_ROWS: /* NOT SUPPORTED, but saved */
            *((SQLLEN *)pvParam) = stmt->options.maxRows;
            MYLOG(DRV_DEBUG, "MAX_ROWS, returning " FORMAT_LEN "\n",
