...
void SC_set_rowset_start(StatementClass *stmt, SQLLEN start, BOOL valid_base) {
    ...
    MYLOG(LOG_DEBUG, "%p->SC_set_rowstart " FORMAT_LEN "->" FORMAT_LEN "(%s) ",
          stmt, stmt->rowset_start, start, valid_base ? "valid" : "unknown");
    if (res != NULL) {
        BOOL valid = QR_has_valid_base(res);
        MYPRINTF(LOG_DEBUG, ":(%p)QR is %s", res,
        ...
        if (!QR_get_cursor(res))
            res->key_base = start;
        MYPRINTF(LOG_DEBUG, ":(%p)QR result=" FORMAT_LEN "(%s)", res,
...
static const struct {
    ...
    {{STMT_FRACTIONAL_TRUNCATED, "01S07", "01S07"},
     ...
     {STMT_PROGRAM_TYPE_OUT_OF_RANGE, "HY003", "S1003"},
