...
RETCODE SQL_API ESAPI_FreeStmt(HSTMT hstmt, SQLUSMALLINT fOption) {
    ...
    MYLOG(DRV_TRACE, "entering...hstmt=%p, fOption=%hi\n", hstmt, fOption);
...
char SC_Destructor(StatementClass *self) {
    ...
    MYLOG(DRV_TRACE, "entering self=%p, self->result=%p, self->hdbc=%p\n", self,
          res, self->hdbc);
    SC_clear_error(self);
...
void SC_set_rowset_start(StatementClass *stmt, SQLLEN start, BOOL valid_base) {
    ...
    MYLOG(DRV_DEBUG, "%p->SC_set_rowstart " FORMAT_LEN "->" FORMAT_LEN "(%s) ",
          stmt, stmt->rowset_start, start, valid_base ? "valid" : "unknown");
    if (res != NULL) {
        BOOL valid = QR_has_valid_base(res);
        MYPRINTF(DRV_DEBUG, ":(%p)QR is %s", res,
        ...
        if (!QR_get_cursor(res))
            res->key_base = start;
        MYPRINTF(DRV_DEBUG, ":(%p)QR result=" FORMAT_LEN "(%s)", res,
                 QR_get_rowstart_in_cache(res),
                 QR_has_valid_base(res) ? "valid" : "unknown");
    }
    stmt->rowset_start = start;
    MYPRINTF(DRV_DEBUG, ":stmt result=" FORMAT_LEN "\n", stmt->rowset_start);
}
...
BOOL SC_opencheck(StatementClass *self, const char *func) {
    ...
    if (self->prepare && self->status == STMT_DESCRIBED) {
        MYLOG(DRV_DEBUG, "self->prepare && self->status == STMT_DESCRIBED\n");
        return FALSE;
    }
...
char SC_recycle_statement(StatementClass *self) {
    ...
    MYLOG(DRV_TRACE, "entering self=%p\n", self);
    ...
    switch (self->prepared) {
        case NOT_PREPARED:
            ...
            MYLOG(DRV_DEBUG, "SC_clear_parse_status\n");
    ...
    self->bind_row = 0;
    MYLOG(DRV_DEBUG, "statement=%p ommitted=0\n", self);
...
void SC_error_copy(StatementClass *self, const StatementClass *from,
    ...
    MYLOG(DRV_TRACE, "entering %p->%p check=%i\n", from, self, check);
    if (!from)
        return; /* for safety */
    if (self == from)
        return; /* for safety */
...
SC_fetch(StatementClass *self) {
    ...
    MYLOG(DRV_TRACE, "entering statement=%p res=%p ommitted=0\n", self, res);
    self->last_fetch_count = self->last_fetch_count_include_ommitted = 0;
    if (!res)
        return SQL_ERROR;
    coli = QR_get_fields(res); /* the column info */

    MYLOG(DRV_DEBUG, "fetch_cursor=%d, %p->total_read=" FORMAT_LEN "\n",
    ...
    MYLOG(DRV_DEBUG, "**** : non-cursor_result\n");
    (self->currTuple)++;
    ...
    MYLOG(DRV_DEBUG, "stmt=%p ommitted++\n", self);
    ...
    for (lf = 0; lf < num_cols; lf++) {
        ...
        if (opts->bindings[lf].buffer != NULL) {
            ...
            MYLOG(DRV_DEBUG, "type = %d, atttypmod = %d\n", type, atttypmod);
            ...
            if (useCursor)
                ...
                MYLOG(DRV_DEBUG, "curt=" FORMAT_LEN "\n", curt);
            ...
            MYLOG(DRV_DEBUG, "copy_and_convert: retval = %d\n", retval);
            ...
            switch (retval) {
                ...
                case COPY_RESULT_TRUNCATED:
                    ...
                    MYLOG(DRV_DEBUG, "The %dth item was truncated\n", lf + 1);
                    MYLOG(DRV_DEBUG, "The buffer size = " FORMAT_LEN,
                          opts->bindings[lf].buflen);
                    MYLOG(DRV_DEBUG, " and the value is '%s'\n", value);
...
RETCODE dequeueNeedDataCallback(RETCODE retcode, StatementClass *stmt) {
    ...
    MYLOG(DRV_TRACE, "entering ret=%d count=%d\n", retcode, stmt->num_callbacks);
...
#define NULLCHECK(a) (a ? a : "(NULL)")
    if (self) {
        ...
        MYLOG(DRV_DEBUG, "%s: func=%s, desc='%s', errnum=%d, errmsg='%s'\n",
              head, func, desc, self->__error_number,
              NULLCHECK(self->__error_message));
        if (SC_get_errornumber(self) > 0) {
            QLOG(level,
        ...
        MYLOG(DRV_DEBUG, "INVALID STATEMENT HANDLE ERROR: func=%s, desc='%s'\n",
              func, desc);
    }
}
...
int SC_Create_bookmark(StatementClass *self, BindInfoClass *bookmark,
    ...
    MYLOG(DRV_TRACE, "entering type=%d buflen=" FORMAT_LEN " buf=%p\n",
    ...
    MYLOG(DRV_TRACE, "leaving cvtlen=" FORMAT_SIZE_T " ix(bl,of)=%d(%d,%d)\n",
