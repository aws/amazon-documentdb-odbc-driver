...
static int setup_getdataclass(SQLLEN *const length_return,
    ...
    if (fCType == SQL_C_WCHAR) {
        if (BYTEA_PROCESS_ESCAPE == bytea_process_kind)
            ...
            MYLOG(DRV_DEBUG, "hybrid estimate\n");
            if ((unicode_count =
    ...
    if (changed || needbuflen > cbValueMax) {
        ...
        if (fCType == SQL_C_WCHAR) {
            if (BYTEA_PROCESS_ESCAPE == bytea_process_kind) {
                ...
                {
                    MYLOG(DRV_DEBUG, "hybrid convert\n");
...
static int convert_text_field_to_sql_c(
    ...
    MYLOG(DRV_DEBUG, "field_type=%u type=%d\n", field_type, fCType);
    ...
    MYLOG(DRV_DEBUG, "DEFAULT: len = " FORMAT_LEN ", ptr = '%.*s'\n", len,
          (int)len, ptr);
...
int copy_and_convert_field(StatementClass *stmt, OID field_type, int atttypmod,
    ...
    if (fCType == SQL_C_DEFAULT) {
        ...
        MYLOG(DRV_DEBUG, ", SQL_C_DEFAULT: fCType = %d\n", fCType);
    }
    ...
    if (text_bin_handling) {
        ...
        switch (field_type) {
            ...
            case ES_TYPE_UUID:
                ...
                midtemp[i] = '\0';
                MYLOG(DRV_DEBUG, "ES_TYPE_UUID: rgbValueBindRow = '%s'\n",
                      rgbValueBindRow);
                break;
        ...
        if (field_type == ES_TYPE_MONEY) {
            if (convert_money(neut_str, midtemp, sizeof(midtemp)))
                ...
                MYLOG(DRV_DEBUG, "couldn't convert money type to %d\n", fCType);
        ...
        switch (fCType) {
            ...
            case SQL_C_BINARY:
                ...
                if (ES_TYPE_INT4 == field_type) {
                    ...
                    MYLOG(DRV_ALL, "SQL_C_VARBOOKMARK value=%d\n", ival);
                    if (pcbValue)
            ...
            case SQL_C_GUID:
                ...
                if (COPY_OK != result) {
                    MYLOG(DRV_DEBUG, "Could not convert to SQL_C_GUID\n");
                    return COPY_UNSUPPORTED_TYPE;
                }
                len = sizeof(g);
                if (bind_size > 0)
            ...
            default:
                MYLOG(DRV_DEBUG, "conversion to the type %d isn't supported\n",
                      fCType);
                return COPY_UNSUPPORTED_TYPE;
        }
    }
...
static size_t convert_from_esbinary(const char *value, char *rgbValue,
    ...
    for (i = 0; i < ilen;) {
        ...
        /** if (rgbValue)
            MYLOG(DRV_DEBUG, "i=%d, rgbValue[%d] = %d, %c\n", i, o, rgbValue[o],
           rgbValue[o]); ***/
    }
    ...
    MYLOG(DRV_DEBUG, "in=" FORMAT_SIZE_T ", out = " FORMAT_SIZE_T "\n", ilen, o);
