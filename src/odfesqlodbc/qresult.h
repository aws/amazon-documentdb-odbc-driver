...
#define QR_set_next_in_cache(self, number)                               \
    do {                                                                 \
        MYLOG(DRV_ALL, "set the number to " FORMAT_LEN " to read next\n", \
              number);                                                   \
        self->fetch_number = number;                                     \
    } while (0)
#define QR_inc_next_in_cache(self)                                             \
    do {                                                                       \
        MYLOG(DRV_ALL, "increased the number " FORMAT_LEN, self->fetch_number); \
        self->fetch_number++;                                                  \
        MYLOG(DRV_ALL, "to " FORMAT_LEN " to next read\n", self->fetch_number); \
    } while (0)
