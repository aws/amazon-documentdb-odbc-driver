...
RETCODE SQL_API SQLAllocHandle(SQLSMALLINT HandleType, SQLHANDLE InputHandle,
    ...
    switch (HandleType) {
        ...
        case SQL_HANDLE_DBC:
            ...
            ret = API_AllocConnect(InputHandle, OutputHandle);
...
RETCODE SQL_API SQLBindParam(HSTMT StatementHandle,
    ...
    SC_set_error(stmt, STMT_NOT_IMPLEMENTED_ERROR,
                 "Timestream does not support parameters.", "SQLBindParam");
    return SQL_ERROR;
}
...
RETCODE SQL_API SQLCloseCursor(HSTMT StatementHandle) {
    ...
    ENTER_STMT_CS(stmt);
    SC_clear_error(stmt);
    ret = API_FreeStmt(StatementHandle, SQL_CLOSE);
    LEAVE_STMT_CS(stmt);
    return ret;
}
...
RETCODE SQL_API SQLFreeHandle(SQLSMALLINT HandleType, SQLHANDLE Handle) {
    ...
    switch (HandleType) {
        case SQL_HANDLE_ENV:
            ret = API_FreeEnv(Handle);
            break;
        case SQL_HANDLE_DBC:
            ret = API_FreeConnect(Handle);
            break;
        case SQL_HANDLE_STMT:
            ...
            ret = API_FreeStmt(Handle, SQL_DROP);
...
RETCODE SQL_API SQLGetStmtAttr(HSTMT StatementHandle, SQLINTEGER Attribute,
    ...
    ENTER_STMT_CS(stmt);
    SC_clear_error(stmt);
    ret = API_GetStmtAttr(StatementHandle, Attribute, Value, BufferLength,
                          StringLength);
    LEAVE_STMT_CS(stmt);
    return ret;
}
...
RETCODE SQL_API SQLSetStmtAttr(HSTMT StatementHandle, SQLINTEGER Attribute,
    ...
    ENTER_STMT_CS(stmt);
    SC_clear_error(stmt);
    ret = API_SetStmtAttr(StatementHandle, Attribute, Value, StringLength);
    LEAVE_STMT_CS(stmt);
    return ret;
}
#endif /* UNICODE_SUPPORTXX */
...
RETCODE SQL_API API_GetFunctions30(HDBC hdbc, SQLUSMALLINT fFunction,
