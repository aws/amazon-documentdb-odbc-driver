...
void QR_inc_rowstart_in_cache(QResultClass *self, SQLLEN base_inc) {
    if (!QR_has_valid_base(self))
        MYLOG(DRV_DEBUG, " called while the cache is not ready\n");
    self->base += base_inc;
    if (QR_synchronize_keys(self))
...
void QR_reset_for_re_execute(QResultClass *self) {
    MYLOG(DRV_TRACE, "entering for %p\n", self);
    if (!self)
        return;
...
TupleField *QR_AddNew(QResultClass *self) {
    ...
    if (!self)
        return NULL;
    MYLOG(DRV_ALL, FORMAT_ULEN "th row(%d fields) alloc=" FORMAT_LEN "\n",
...
void QR_free_memory(QResultClass *self) {
    ...
    MYLOG(DRV_TRACE, "entering fcount=" FORMAT_LEN "\n", num_backend_rows);
