...
static int mylog_on = DRV_WARNING, qlog_on = DRV_WARNING;
...
void logs_on_off(int cnopen, int mylog_onoff, int qlog_onoff) {
    ...
    LEAVE_QLOG_CS;
    MYLOG(DRV_DEBUG, "mylog_on=%d qlog_on=%d\n", mylog_on, qlog_on);
}
...
void logInstallerError(int ret, const char *dir) {
    ...
    if (msg[0] != '\0')
        MYLOG(DRV_DEBUG, "Dir= %s ErrorMsg = %s\n", dir, msg);
}
...
void InitializeLogging(void) {
    ...
    qlog_initialize();
    start_logging();
    MYLOG(DRV_DEBUG, "Log Output Dir: %s\n", logdir);
}
