...
int get_convtype(void) {
        ...
        switch (sizeof_w) {
            case 2:
                if ('a' == cdt[0] && '\0' == cdt[1] && '\0' == cdt[2]
                    && '\0' == cdt[3]) {
                    MYLOG(DRV_DEBUG, " UTF-16LE detected\n");
            ...
            case 4:
                if ('a' == cdt[0] && '\0' == cdt[1] && '\0' == cdt[2]
                    ...
                    && '\0' == cdt[6] && '\0' == cdt[7]) {
                    MYLOG(DRV_DEBUG, " UTF32-LE detected\n");
    ...
    if (convtype < 0) {
        ...
        if ('a' == cdt[0] && '\0' == cdt[1] && '\0' == cdt[2]
            && '\0' == cdt[3]) {
            MYLOG(DRV_DEBUG, " C16_UTF-16LE detected\n");
...
char *ucs2_to_utf8(const SQLWCHAR *ucs2str, SQLLEN ilen, SQLLEN *olen,
    ...
    char *utf8str;
    int len = 0;
    MYLOG(DRV_DEBUG, "%p ilen=" FORMAT_LEN " ", ucs2str, ilen);
...
utf8_to_ucs2_lf(const char *utf8str, SQLLEN ilen, BOOL lfconv,
    ...
    MYLOG(DRV_DEBUG, "ilen=" FORMAT_LEN " bufcount=" FORMAT_ULEN, ilen,
          bufcount);
    if (!utf8str)
        return 0;
    MYPRINTF(DRV_DEBUG, " string=%s", utf8str);
...
cleanup:
    ...
    if (ocount < bufcount && ucs2str)
        ucs2str[ocount] = 0;
    MYPRINTF(DRV_ALL, " ocount=" FORMAT_ULEN "\n", ocount);
    return rtn;
}
...
static char *ucs4_to_utf8(const UInt4 *ucs4str, SQLLEN ilen, SQLLEN *olen,
    ...
    char *utf8str;
    int len = 0;
    MYLOG(DRV_DEBUG, " %p ilen=" FORMAT_LEN "\n", ucs4str, ilen);
    ...
    MYLOG(DRV_DEBUG, " newlen=" FORMAT_LEN "\n", ilen);
    utf8str = (char *)malloc(ilen * 4 + 1);
    if (utf8str) {
        int i;
        UInt2 byte2code;
...
static SQLULEN utf8_to_ucs4_lf(const char *utf8str, SQLLEN ilen, BOOL lfconv,
    ...
    MYLOG(DRV_DEBUG, " ilen=" FORMAT_LEN " bufcount=" FORMAT_ULEN "\n", ilen, bufcount);
    if (!utf8str)
        return 0;
    MYLOG(99, " string=%s\n", utf8str);
...
cleanup:
    ...
    if (ocount < bufcount && ucs4str)
        ucs4str[ocount] = 0;
    MYLOG(DRV_DEBUG, " ocount=" FORMAT_ULEN "\n", ocount);
    return rtn;
}
...
static int ucs4_to_ucs2_lf(const unsigned int *ucs4str, SQLLEN ilen,
    ...
    MYLOG(DRV_DEBUG, " ilen=" FORMAT_LEN " bufcount=%d\n", ilen, bufcount);
    if (ilen < 0)
        ilen = ucs4strlen(ucs4str);
...
static int msgtowstr(const char *inmsg, wchar_t *outmsg, int buflen) {
    ...
    MYLOG(DRV_DEBUG, " inmsg=%p buflen=%d\n", inmsg, buflen);
#ifdef WIN32
    if (NULL == outmsg)
        buflen = 0;
    if ((outlen =
    ...
    if (outmsg && outlen >= buflen) {
        outmsg[buflen - 1] = 0;
        MYLOG(DRV_DEBUG, " out=%dchars truncated to %d\n", outlen, buflen - 1);
    }
    MYLOG(DRV_DEBUG, " buf=%dchars out=%dchars\n", buflen, outlen);
...
static int wstrtomsg(const wchar_t *wstr, char *outmsg, int buflen) {
    ...
    MYLOG(DRV_DEBUG, " wstr=%p buflen=%d\n", wstr, buflen);
#ifdef WIN32
    if (NULL == outmsg)
        buflen = 0;
    ...
    if (outmsg && outlen >= buflen) {
        outmsg[buflen - 1] = 0;
        MYLOG(DRV_DEBUG, " out=%dbytes truncated to %d\n", outlen, buflen - 1);
    }
    MYLOG(DRV_DEBUG, " buf=%dbytes outlen=%dbytes\n", buflen, outlen);
...
static SQLLEN mbstoc16_lf(char16_t *c16dt, const char *c8dt, size_t n,
    ...
    MYLOG(DRV_DEBUG, " c16dt=%p size=" FORMAT_SIZE_T "\n", c16dt, n);
...
static SQLLEN c16tombs(char *c8dt, const char16_t *c16dt, size_t n) {
    ...
    MYLOG(DRV_DEBUG, " c8dt=%p size=" FORMAT_SIZE_T "u\n", c8dt, n);
    if (!c8dt)
        n = 0;
...
SQLLEN bindcol_hybrid_exec(SQLWCHAR *utf16, const char *ldt, size_t n,
    ...
    get_convtype();
    MYLOG(DRV_DEBUG, " size=" FORMAT_SIZE_T " lf_conv=%d\n", n, lf_conv);
#if defined(__WCS_ISO10646__)
    if (use_wcs) {
        unsigned int *utf32 = NULL;
...
SQLLEN bindcol_localize_estimate(const char *utf8dt, BOOL lf_conv,
    ...
    MYLOG(DRV_DEBUG, " return=" FORMAT_LEN "\n", l);
    return l;
}
...
SQLLEN bindcol_localize_exec(char *ldt, size_t n, BOOL lf_conv, char **wcsbuf) {
    ...
    get_convtype();
    MYLOG(DRV_DEBUG, " size=" FORMAT_SIZE_T "\n", n);
#if defined(__WCS_ISO10646__)
    if (use_wcs) {
