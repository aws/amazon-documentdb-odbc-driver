...
RETCODE SQL_API API_CopyDesc(SQLHDESC SourceDescHandle,
    ...
    switch (srchd->desc_type) {
        case SQL_ATTR_APP_ROW_DESC:
            MYLOG(LOG_DEBUG, "src=%p target=%p type=%d", src, target,
                  srchd->desc_type);
            if (!targethd->type_defined) {
...
static ErrorInfo *DC_create_errorinfo(const DescriptorClass *self) {
...
RETCODE SQL_API API_DescError(SQLHDESC hdesc, SQLSMALLINT RecNumber,
