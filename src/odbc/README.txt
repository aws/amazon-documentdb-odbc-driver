...
Once you have built and installed Ignite ODBC Driver i.e. libdocumentdb-odbc.so it
...
1. Ensure linker is able to locate all dependencies of the ODBC driver. You
   ...
   under /usr/local/lib):
   $ ldd /usr/local/lib/libdocumentdb-odbc.so.
...
2. Edit file $IGNITE_HOME/platforms/cpp/odbc/install/documentdb-odbc-install.ini
   and ensure that "Driver" parameter of the "Apache Ignite" section points
   to the right location where libdocumentdb-odbc.so is located.

3. To install Apache Ignite ODBC driver use the following command:
   $ odbcinst -i -d -f $IGNITE_HOME/platforms/cpp/odbc/install/documentdb-odbc-install.ini
