...
const std::string ConnectionStringParser::Key::loginTimeoutSec =
    "login_timeout_sec";
const std::string ConnectionStringParser::Key::readPreference =
    "read_preference";
...
const std::string ConnectionStringParser::Key::tlsAllowInvalidHostnames =
...
const std::string ConnectionStringParser::Key::sshPrivateKeyFile =
    "ssh_private_key_file";
const std::string ConnectionStringParser::Key::sshPrivateKeyPassphrase =
    "ssh_private_key_passphrase";
const std::string ConnectionStringParser::Key::sshStrictHostKeyChecking =
...
const std::string ConnectionStringParser::Key::defaultFetchSize =
    "default_fetch_size";
  ...
  if (lKey == Key::dsn) {
    ...
    if (!common::AllDigits(value)) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
            MakeErrorMessage(
                "Port attribute value contains unexpected characters."
                " Using default value.",
                key, value));
      }
    ...
    if (!common::AllDigits(value)) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
            MakeErrorMessage("Login timeout seconds attribute value contains "
    ...
    if (value.size() >= sizeof(std::to_string(UINT32_MAX))) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
            MakeErrorMessage("Login timeout seconds attribute value is too "
    ...
    if (numValue <= 0 || numValue > UINT32_MAX) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
            MakeErrorMessage(
                "Login timeout seconds attribute value is out of range."
                " Using default value.",
                key, value));
      }
    ...
    if (!common::AllDigits(value)) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
            MakeErrorMessage(
                "Scan limit attribute value contains unexpected characters."
                " Using default value.",
                key, value));
      }
    ...
    if (!common::AllDigits(value)) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
            MakeErrorMessage("Default fetch size attribute value contains "
                             "unexpected characters."
                             " Using default value.",
                             key, value));
      }
    ...
    if (numValue <= 0 || numValue > UINT32_MAX) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
            MakeErrorMessage(
                "Default fetch size attribute value is out of range."
                " Using default value.",
                key, value));
      }
