...
BigInteger::BigInteger(int64_t val) : sign(1), mag() {
  AssignInt64(val);
}
...
void BigInteger::MagnitudeToBytes(
    common::FixedSizeArray< int8_t >& buffer) const {
 ...
 * @return Carry.
 */
uint32_t MultiplyAndSubstruct(uint32_t* q, const uint32_t* a, int32_t alen,
                              uint32_t x) {
  uint64_t carry = 0;
...
void BigInteger::Divide(const BigInteger& divisor, BigInteger& res,
  ...
  for (int32_t i = ulen - vlen; i >= 0; --i) {
    ...
    while (qhat > UINT32_MAX
           || ((qhat * nv[vlen - 2])
               > ((UINT32_MAX + static_cast< uint64_t >(1)) * rhat
                  + nu[i + vlen - 2]))) {
      --qhat;
      rhat += nv[vlen - 1];
    ...
    uint32_t carry =
        MultiplyAndSubstruct(nu.GetData() + i, nv.GetData(), vlen, qhat32);
