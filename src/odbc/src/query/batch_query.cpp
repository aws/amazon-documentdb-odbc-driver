...
SqlResult::Type BatchQuery::Execute() {
  ...
  do {
    ...
    res = MakeRequestExecuteBatch(processed, processed + currentPageSize,
                                  lastPage);

    processed += currentPageSize;
  } while (
      (res == SqlResult::AI_SUCCESS || res == SqlResult::AI_SUCCESS_WITH_INFO)
      && processed < rowNum);
...
SqlResult::Type BatchQuery::MakeRequestExecuteBatch(SqlUlen begin, SqlUlen end,
  ...
  for (size_t i = 0; i < rowsLastTime.size(); ++i) {
    ...
    params.SetParamStatus(
        idx, rowsLastTime[i] < 0 ? SQL_PARAM_ERROR : SQL_PARAM_SUCCESS);
  }
