 ...
 */
struct ProcessorOp {
  enum Type { GET_BINARY_PROCESSOR = 21, RELEASE_START = 22 };
};
...
class ClusterNodesHolder {
 ...
 public:
  ...
  void AddNode(SP_ClusterNodeImpl node) {
    ...
    nodes.insert(
        std::pair< Guid, SP_ClusterNodeImpl >(node.Get()->GetId(), node));
  }
...
int64_t IGNITE_CALL InLongOutLong(void* target, int type, int64_t val) {
  ...
  switch (type) {
    case OperationCallback::NODE_INFO: {
      ...
      SharedPointer< InteropMemory > memCopy(
          new InteropUnpooledMemory(mem.Get()->Capacity()));
...
IgniteEnvironment::IgniteEnvironment(const IgniteConfiguration& cfg)
    : cfg(new IgniteConfiguration(cfg)),
      ...
      name(0),
      proc(),
      registry(DEFAULT_FAST_PATH_CONTAINERS_CAP,
               DEFAULT_SLOW_PATH_CONTAINERS_CAP),
...
void IgniteEnvironment::Initialize() {
  ...
  jobject binaryProc = Context()->TargetOutObject(
      proc.Get(), ProcessorOp::GET_BINARY_PROCESSOR, &jniErr);
  ...
  ignite = new Ignite(new IgniteImpl(
      SharedPointer< IgniteEnvironment >(this, SharedPointerEmptyDeleter)));
}
...
SharedPointer< InteropMemory > IgniteEnvironment::AllocateMemory() {
  SharedPointer< InteropMemory > ptr(
      new InteropUnpooledMemory(DEFAULT_ALLOCATION_SIZE));
...
void IgniteEnvironment::ComputeJobExecuteLocal(int64_t jobHandle) {
  ...
  if (job)
    ...
    IGNITE_ERROR_FORMATTED_1(
        DocumentDbError::IGNITE_ERR_COMPUTE_USER_UNDECLARED_EXCEPTION,
        "Job is not registred for handle", "jobHandle", jobHandle);
  }
}

int32_t IgniteEnvironment::ComputeTaskLocalJobResult(int64_t taskHandle,
                                                     int64_t jobHandle) {
  SharedPointer< compute::ComputeJobHolder > job0 =
      StaticPointerCast< compute::ComputeJobHolder >(registry.Get(jobHandle));
...
int64_t IgniteEnvironment::ComputeJobCreate(
  ...
  if (!binding.Get())
    throw DocumentDbError(DocumentDbError::IGNITE_ERR_UNKNOWN,
  ...
  BinaryObjectImpl binJob =
      BinaryObjectImpl::FromMemory(*mem.Get(), inStream.Position(), 0);

  int32_t jobTypeId = binJob.GetTypeId();

  bool invoked = false;

  int64_t handle = binding.Get()->InvokeCallback(
      invoked, IgniteBindingImpl::CallbackType::COMPUTE_JOB_CREATE, jobTypeId,
      reader, writer);
...
int32_t IgniteEnvironment::ComputeTaskJobResult(
    SharedPointer< InteropMemory >& mem) {
  InteropInputStream inStream(mem.Get());
  BinaryReaderImpl reader(&inStream);
...
void IgniteEnvironment::ProcessorReleaseStart() {
  if (proc.Get()) {
    JniErrorInfo jniErr;
    ctx.Get()->TargetInLongOutLong(proc.Get(), ProcessorOp::RELEASE_START, 0,
                                   &jniErr);
  }
}
...
void IgniteEnvironment::OnContinuousQueryListenerApply(
    SharedPointer< InteropMemory >& mem) {
  ...
  ContinuousQueryImplBase* contQry =
      reinterpret_cast< ContinuousQueryImplBase* >(
          registry.Get(qryHandle).Get());
...
int64_t IgniteEnvironment::OnContinuousQueryFilterCreate(
  ...
  BinaryObjectImpl binFilter =
      BinaryObjectImpl::FromMemory(*mem.Get(), inStream.Position(), metaMgr);

  int32_t filterId = binFilter.GetTypeId();

  bool invoked = false;

  int64_t res = binding.Get()->InvokeCallback(
      invoked, IgniteBindingImpl::CallbackType::CACHE_ENTRY_FILTER_CREATE,
...
int64_t IgniteEnvironment::OnContinuousQueryFilterApply(
    SharedPointer< InteropMemory >& mem) {
  InteropInputStream inStream(mem.Get());
  BinaryReaderImpl reader(&inStream);
  BinaryRawReader rawReader(&reader);
...
int64_t IgniteEnvironment::OnFuturePrimitiveResult(int64_t handle,
                                                   int64_t value) {
  SharedPointer< compute::ComputeTaskHolder > task0 =
...
int64_t IgniteEnvironment::OnFutureObjectResult(
    int64_t handle, SharedPointer< InteropMemory >& mem) {
  InteropInputStream inStream(mem.Get());
  BinaryReaderImpl reader(&inStream);
...
int64_t IgniteEnvironment::OnFutureError(int64_t handle,
                                         SharedPointer< InteropMemory >& mem) {
  InteropInputStream inStream(mem.Get());
  BinaryReaderImpl reader(&inStream);
  BinaryRawReader rawReader(&reader);
  ...
  DocumentDbError err(DocumentDbError::IGNITE_ERR_GENERIC, errStr.c_str());
...
int64_t IgniteEnvironment::OnComputeFuncExecute(
    SharedPointer< InteropMemory >& mem) {
  InteropInputStream inStream(mem.Get());
  BinaryReaderImpl reader(&inStream);
  ...
  if (local) {
    ...
    handle = binding.Get()->InvokeCallback(
        invoked, IgniteBindingImpl::CallbackType::COMPUTE_JOB_CREATE, jobTypeId,
        reader, writer);
...
void IgniteEnvironment::CacheInvokeCallback(
  ...
  if (local)
    throw DocumentDbError(DocumentDbError::IGNITE_ERR_UNSUPPORTED_OPERATION,
  ...
  binding.Get()->InvokeCallback(
      invoked, IgniteBindingImpl::CallbackType::CACHE_ENTRY_PROCESSOR_APPLY,
