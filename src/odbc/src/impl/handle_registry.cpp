...
void HandleRegistrySegment::Put(int64_t hnd,
                                const SharedPointer< void >& entry) {
  CsLockGuard guard(mux);
...
int64_t HandleRegistry::AllocateCriticalSafe(
    const SharedPointer< void >& target) {
...
int64_t HandleRegistry::Allocate0(const SharedPointer< void >& target,
  ...
  // Either allocating on slow-path, or fast-path can no longer accomodate more
  // entries.
