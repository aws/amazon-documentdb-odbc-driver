...
ClusterNodeImpl::ClusterNodeImpl(SharedPointer< InteropMemory > mem)
    : mem(mem),
      addrs(new std::vector< std::string >),
      attrs(new std::map< std::string, int32_t >),
      hosts(new std::vector< std::string >),
      isClient(false),
      isDaemon(false),
      isLocal(false),
      consistentId(new std::string) {
...
std::vector< std::string > ClusterNodeImpl::GetAttributes() const {
  ...
  for (std::map< std::string, int32_t >::const_iterator it =
           attrs.Get()->begin();
       it != attrs.Get()->end(); ++it)
...
void ClusterNodeImpl::ReadProductVersion(BinaryReaderImpl& reader) {
  ...
  ver = SharedPointer< IgniteProductVersion >(new IgniteProductVersion(
      major, minor, maintenance, stage, releaseDate, revHash));
}
}  // namespace cluster
}  // namespace impl
}  // namespace odbc
