...
std::vector< int32_t > CacheAffinityImpl::GetPrimaryPartitions(
    ClusterNode node) {
  return GetPartitions(Command::PRIMARY_PARTITIONS, node);
}

std::vector< int32_t > CacheAffinityImpl::GetBackupPartitions(
    ClusterNode node) {
...
std::map< int32_t, ClusterNode > CacheAffinityImpl::MapPartitionsToNodes(
...
std::vector< ClusterNode > CacheAffinityImpl::MapPartitionToPrimaryAndBackups(
    int32_t part) {
  SharedPointer< interop::InteropMemory > memIn =
      GetEnvironment().AllocateMemory();
  SharedPointer< interop::InteropMemory > memOut =
...
std::vector< int32_t > CacheAffinityImpl::GetPartitions(int32_t opType,
                                                        ClusterNode node) {
  SharedPointer< interop::InteropMemory > memIn =
      GetEnvironment().AllocateMemory();
  SharedPointer< interop::InteropMemory > memOut =
