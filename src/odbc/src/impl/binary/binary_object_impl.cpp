...
BinaryObjectImpl::BinaryObjectImpl(interop::InteropMemory& mem, int32_t start,
    ...
    : mem(&mem), start(start), idRslvr(0), metaMgr(metaMgr), binary(false) {
  if (idRslvr)
...
BinaryObjectImpl::BinaryObjectImpl(const BinaryObjectImpl& other)
    : mem(other.mem), start(other.start), idRslvr(0), metaMgr(other.metaMgr) {
  if (other.idRslvr)
...
BinaryObjectImpl BinaryObjectImpl::FromMemory(interop::InteropMemory& mem,
                                              int32_t offset,
                                              BinaryTypeManager* metaMgr) {
...
int32_t BinaryObjectImpl::FindField(const int32_t fieldId) const {
  ...
  if (flags & IGNITE_BINARY_FLAG_OFFSET_ONE_BYTE) {
    ...
    for (int32_t schemaPos = footerBegin; schemaPos < footerEnd;
      ...
      if (fieldId == currentFieldId)
        return (BinaryUtils::UnsafeReadInt16(*mem, schemaPos + 4) & 0xFFFF)
               + start;
    }
  } else {
    for (int32_t schemaPos = footerBegin; schemaPos < footerEnd;
