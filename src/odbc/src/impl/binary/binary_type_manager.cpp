...
SharedPointer< BinaryTypeHandler > BinaryTypeManager::GetHandler(
  ...
  {  // Locking scope.
    ...
    if (it != snapshots->end())
      return SharedPointer< BinaryTypeHandler >(
          new BinaryTypeHandler(it->second));
...
bool BinaryTypeManager::ProcessPendingUpdates(IgniteError& err) {
  ...
  for (std::vector< SPSnap >::iterator it = pending->begin();
       it != pending->end(); ++it) {
