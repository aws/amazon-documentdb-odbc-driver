...
void Parameter::Write(impl::binary::BinaryWriterImpl& writer, int offset,
                      SqlUlen idx) const {
  ...
  if (buffer.IsDataAtExec()) {
    buf = ApplicationDataBuffer(buffer.GetType(),
                                const_cast< int8_t* >(&storedData[0]),
                                storedDataLen, &storedDataLen);
  }
  ...
  switch (sqlType) {
    ...
    case SQL_LONGVARBINARY: {
      ...
      writer.WriteInt8Array(
          reinterpret_cast< const int8_t* >(constRef.GetData()), paramLen);
...
bool Parameter::IsDataReady() const {
  return !buffer.IsDataAtExec()
         || static_cast< SqlLen >(storedData.size())
                == buffer.GetDataAtExecSize();
}
