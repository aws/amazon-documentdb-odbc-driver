...
void ParameterSet::Prepare() {
  ...
  for (ParameterBindingMap::iterator it = parameters.begin();
       it != parameters.end(); ++it)
    it->second.ResetStoredData();
}

bool ParameterSet::IsDataAtExecNeeded() const {
  for (ParameterBindingMap::const_iterator it = parameters.begin();
       it != parameters.end(); ++it) {
...
void ParameterSet::Write(impl::binary::BinaryWriterImpl& writer, SqlUlen begin,
...
void ParameterSet::WriteRow(impl::binary::BinaryWriterImpl& writer,
                            SqlUlen idx) const {
  uint16_t prev = 0;
