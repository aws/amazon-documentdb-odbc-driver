...
const static std::string WORD_PER_NODE_PARALLEL_OPERATIONS(
...
void SqlSetStreamingCommand::Parse(SqlLexer& lexer) {
  ...
  while (*lexer.Shift() && token.GetType() != TokenType::SEMICOLON) {
    ...
    if (token.ToLower() == WORD_PER_NODE_PARALLEL_OPERATIONS) {
      ...
      parallelOpsPerNode =
          ExpectPositiveInteger(lexer, "per node parallel operations number");
    ...
    ThrowUnexpectedTokenError(
        token, "additional parameter of SET STREAMING command or semicolon");
  }
}
...
void SqlSetStreamingCommand::ThrowUnexpectedTokenError(
    const SqlToken& token, const std::string& expected) {
  throw OdbcError(SqlState::S42000_SYNTAX_ERROR_OR_ACCESS_VIOLATION,
                  "Unexpected token: '" + token.ToString() + "', " + expected
                      + " expected.");
}

void SqlSetStreamingCommand::ThrowUnexpectedEndOfStatement(
...
int32_t SqlSetStreamingCommand::ExpectPositiveInteger(
    SqlLexer& lexer, const std::string& description) {
  int32_t val = ExpectInt(lexer);

  if (val <= 0) {
    throw OdbcError(
        SqlState::S42000_SYNTAX_ERROR_OR_ACCESS_VIOLATION,
