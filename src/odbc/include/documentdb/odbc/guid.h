...
/**
 * @file
 * Declares documentdb::odbc::Guid class.
 */

#ifndef _DOCUMENTDB_ODBC_GUID
#define _DOCUMENTDB_ODBC_GUID
...
::std::basic_ostream< C >& operator<<(std::basic_ostream< C >& os,
  ...
  uint16_t part4 =
      static_cast< uint16_t >(guid.GetLeastSignificantBits() >> 48);
  uint64_t part5 = guid.GetLeastSignificantBits() & 0x0000FFFFFFFFFFFFU;

  os << std::hex << std::setfill< C >('0') << std::setw(8) << part1 << '-'
...
::std::basic_istream< C >& operator>>(std::basic_istream< C >& is, Guid& guid) {
  ...
  guid = Guid((parts[0] << 32) | (parts[1] << 16) | parts[2],
              (parts[3] << 48) | parts[4]);
