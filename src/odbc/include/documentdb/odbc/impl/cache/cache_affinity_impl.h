...
#ifndef _DOCUMENTDB_ODBC_CACHE_AFFINITY_IMPL
#define _DOCUMENTDB_ODBC_CACHE_AFFINITY_IMPL
...
/* Shared pointer. */
typedef common::concurrent::SharedPointer< CacheAffinityImpl >
    SP_CacheAffinityImpl;
...
class IGNITE_FRIEND_EXPORT CacheAffinityImpl : private interop::InteropTarget {
 ...
 public:
  ...
  /**
   * Get number of partitions in cache according to configured affinity
   * function.
   *
  ...
  int32_t GetPartition(const K& key) {
    ...
    int32_t ret =
        static_cast< int32_t >(InStreamOutLong(Command::PARTITION, inOp, err));
   ...
   */
  template < typename K >
  bool IsPrimary(documentdb::odbc::cluster::ClusterNode node, const K& key) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
   ...
   */
  template < typename K >
  bool IsBackup(documentdb::odbc::cluster::ClusterNode node, const K& key) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
        GetEnvironment().AllocateMemory();
    interop::InteropOutputStream out(memIn.Get());
    binary::BinaryWriterImpl writer(&out, GetEnvironment().GetTypeManager());
   ...
   */
  template < typename K >
  bool IsPrimaryOrBackup(documentdb::odbc::cluster::ClusterNode node,
                         const K& key) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
        GetEnvironment().AllocateMemory();
    interop::InteropOutputStream out(memIn.Get());
   ...
   *
   * @param node Cluster node.
   * @return Container of partition ids for which the given cluster node has
   * primary ownership.
   */
  std::vector< int32_t > GetPrimaryPartitions(
      documentdb::odbc::cluster::ClusterNode node);

  /**
   * Get partition ids for which given cluster node has backup ownership.
   *
   * @param node Cluster node.
   * @return Container of partition ids for which given cluster node has backup
   * ownership.
   */
  std::vector< int32_t > GetBackupPartitions(
      documentdb::odbc::cluster::ClusterNode node);

  /**
   * Get partition ids for which given cluster node has any ownership (either
   * primary or backup).
   *
   * @param node Cluster node.
   * @return Container of partition ids for which given cluster node has any
   * ownership (either primary or backup).
   */
  std::vector< int32_t > GetAllPartitions(
      documentdb::odbc::cluster::ClusterNode node);
   ...
   * @return Key to be used for node-to-affinity mapping (may be the same key as
   * passed in).
   */
   ...
   * nodes. Use it to determine which nodes are storing which keys prior to
   * sending jobs that access these keys.
   *
   * @tparam TK Key to map type.
   *
   * @param keys Keys to map to nodes.
   * @return Map of nodes to keys or empty map if there are no alive nodes for
   * this cache.
   */
  template < typename TK >
  std::map< documentdb::odbc::cluster::ClusterNode, std::vector< TK > >
  MapKeysToNodes(const std::vector< TK >& keys) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
        GetEnvironment().AllocateMemory();
    common::concurrent::SharedPointer< interop::InteropMemory > memOut =
        GetEnvironment().AllocateMemory();
    interop::InteropOutputStream out(memIn.Get());
    binary::BinaryWriterImpl writer(&out, GetEnvironment().GetTypeManager());

    writer.WriteInt32(static_cast< int32_t >(keys.size()));
    for (typename std::vector< TK >::const_iterator it = keys.begin();
         it != keys.end(); ++it)
    ...
    DocumentDbError err;
    InStreamOutStream(Command::MAP_KEYS_TO_NODES, *memIn.Get(), *memOut.Get(),
                      err);
    DocumentDbError::ThrowIfNeeded(err);
    ...
    std::map< documentdb::odbc::cluster::ClusterNode, std::vector< TK > > ret;

    int32_t cnt = reader.ReadInt32();
    for (int32_t i = 0; i < cnt; i++) {
      std::vector< TK > val;

      documentdb::odbc::cluster::ClusterNode key(
          GetEnvironment().GetNode(reader.ReadGuid()));
      reader.ReadCollection< TK >(std::back_inserter(val));

      ret.insert(
          std::pair< documentdb::odbc::cluster::ClusterNode, std::vector< TK > >(
              key, val));
    }
   ...
   * is mapped. Use it to determine which nodes are storing which keys prior to
   * sending jobs that access these keys.
   *
   * @tparam TK Key to map type.
   *
   ...
   */
  template < typename TK >
  documentdb::odbc::cluster::ClusterNode MapKeyToNode(const TK& key) {
    Guid nodeId;
    In1Operation< TK > inOp(key);
   ...
   */
  template < typename TK >
  std::vector< documentdb::odbc::cluster::ClusterNode > MapKeyToPrimaryAndBackups(
      const TK& key) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
        GetEnvironment().AllocateMemory();
    common::concurrent::SharedPointer< interop::InteropMemory > memOut =
    ...
    DocumentDbError err;
    InStreamOutStream(Command::MAP_KEY_TO_PRIMARY_AND_BACKUPS, *memIn.Get(),
                      *memOut.Get(), err);
    DocumentDbError::ThrowIfNeeded(err);
    ...
    std::vector< documentdb::odbc::cluster::ClusterNode > ret;
    int32_t cnt = reader.ReadInt32();
    ret.reserve(cnt);
    for (int32_t i = 0; i < cnt; i++)
   ...
   */
  documentdb::odbc::cluster::ClusterNode MapPartitionToNode(int32_t part);
   ...
   */
  std::map< int32_t, documentdb::odbc::cluster::ClusterNode > MapPartitionsToNodes(
   ...
   * @return Collection of primary and backup nodes for partition with primary
   * node always first.
   */
  std::vector< documentdb::odbc::cluster::ClusterNode >
  MapPartitionToPrimaryAndBackups(int32_t part);
 ...
 private:
   ...
   */
  std::vector< int32_t > GetPartitions(int32_t opType,
                                       documentdb::odbc::cluster::ClusterNode node);
};
}  // namespace cache
}  // namespace impl
...
#endif  // _DOCUMENTDB_ODBC_CACHE_AFFINITY_IMPL
