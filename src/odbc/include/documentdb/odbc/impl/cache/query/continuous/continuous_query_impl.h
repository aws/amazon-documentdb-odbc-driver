...
/**
 * @file
 * Declares documentdb::odbc::impl::cache::query::continuous::ContinuousQueryImpl
 * class.
 */

#ifndef _DOCUMENTDB_ODBC_IMPL_CACHE_QUERY_CONTINUOUS_CONTINUOUS_QUERY_IMPL
#define _DOCUMENTDB_ODBC_IMPL_CACHE_QUERY_CONTINUOUS_CONTINUOUS_QUERY_IMPL

#include <stdint.h>
#include <memory>

#include <documentdb/odbc/reference.h>

#include <documentdb/odbc/cache/event/cache_entry_event_listener.h>
#include <documentdb/odbc/binary/binary_raw_reader.h>
#include <documentdb/odbc/impl/cache/event/cache_entry_event_filter_holder.h>
 ...
 * documentdb::odbc::cache::Cache::QueryContinuous().
 */
class ContinuousQueryImplBase {
 public:
   ...
   */
  explicit ContinuousQueryImplBase(
      bool loc, event::CacheEntryEventFilterHolderBase* filterOp)
      : local(loc),
   ...
   */
  virtual void ReadAndProcessEvents(
      documentdb::odbc::binary::BinaryRawReader& reader) = 0;
...
class ContinuousQueryImpl : public ContinuousQueryImplBase {
 public:
   ...
   */
  ContinuousQueryImpl(
      Reference< documentdb::odbc::cache::event::CacheEntryEventListener< K, V > >&
          lsnr,
      bool loc)
      : ContinuousQueryImplBase(
          loc, new event::CacheEntryEventFilterHolder< void >()),
        lsnr(lsnr) {
    // No-op.
  }
  ...
  ContinuousQueryImpl(
      ...
      : ContinuousQueryImplBase(
          loc, new event::CacheEntryEventFilterHolder< F >(filter)),
        lsnr(lsnr) {
    // No-op.
  }
   ...
   */
  const documentdb::odbc::cache::event::CacheEntryEventListener< K, V >&
  GetListener() const {
    return lsnr.Get();
  }
   ...
   * @return Cache entry event listener.
   */
  documentdb::odbc::cache::event::CacheEntryEventListener< K, V >& GetListener() {
    return lsnr.Get();
  }
   ...
   */
  virtual void ReadAndProcessEvents(
      documentdb::odbc::binary::BinaryRawReader& reader) {
    // Number of events.
    int32_t cnt = reader.ReadInt32();

    // Storing events here.
    std::vector< documentdb::odbc::cache::CacheEntryEvent< K, V > > events;
    events.resize(cnt);
 ...
 private:
  ...
  Reference< documentdb::odbc::cache::event::CacheEntryEventListener< K, V > > lsnr;
};
...
class RemoteFilterHolder : public ContinuousQueryImplBase {
 public:
  ...
  RemoteFilterHolder(const Reference< F >& filter)
      : ContinuousQueryImplBase(
          false, new event::CacheEntryEventFilterHolder< F >(filter)) {
    // No-op.
  }
...
#endif  //_DOCUMENTDB_ODBC_IMPL_CACHE_QUERY_CONTINUOUS_CONTINUOUS_QUERY_IMPL
