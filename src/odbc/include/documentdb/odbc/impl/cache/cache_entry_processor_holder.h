...
#ifndef _DOCUMENTDB_ODBC_IMPL_CACHE_CACHE_ENTRY_PROCESSOR_HOLDER
#define _DOCUMENTDB_ODBC_IMPL_CACHE_CACHE_ENTRY_PROCESSOR_HOLDER

#include <documentdb/odbc/common/common.h>
#include <documentdb/odbc/cache/mutable_cache_entry.h>
 ...
 */
template < typename V >
MutableCacheEntryState::Type GetMutableCacheEntryState(const V& valueBefore,
                                                       bool existsBefore,
                                                       const V& valueAfter,
                                                       bool existsAfter) {
...
/**
 * Holder for the Cache Entry Processor and its argument. Used as a convenient
 * way to transmit Cache Entry Processor between nodes.
 */
...
class CacheEntryProcessorHolder {
 public:
  ...
  R Process(const K& key, V& value, bool exists,
            MutableCacheEntryState::Type& state) {
    typedef documentdb::odbc::cache::MutableCacheEntry< K, V > Entry;
    ...
    state = GetMutableCacheEntryState(value, exists, entry.GetValue(),
                                      entry.IsExists());
...
#endif  //_DOCUMENTDB_ODBC_IMPL_CACHE_CACHE_ENTRY_PROCESSOR_HOLDER
