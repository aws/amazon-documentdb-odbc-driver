...
/**
 * @file
 * Declares documentdb::odbc::impl::compute::ComputeImpl class.
 */

#ifndef _DOCUMENTDB_ODBC_IMPL_COMPUTE_COMPUTE_IMPL
#define _DOCUMENTDB_ODBC_IMPL_COMPUTE_COMPUTE_IMPL
...
#include <documentdb/odbc/impl/compute/java_compute_task_holder.h>
#include <documentdb/odbc/impl/compute/single_job_compute_task_holder.h>
#include <documentdb/odbc/impl/compute/multiple_job_compute_task_holder.h>
...
class DOCUMENTDB_IMPORT_EXPORT ComputeImpl : public interop::InteropTarget {
 public:
   ...
   *  it's ready.
   * @throw DocumentDbError in case of error.
   */
  template < typename R, typename K, typename F >
  Future< R > AffinityCallAsync(const std::string& cacheName, const K& key,
                                const F& func) {
    typedef ComputeJobHolderImpl< F, R > JobType;
    typedef SingleJobComputeTaskHolder< F, R > TaskType;

    return PerformAffinityTask< R, K, F, JobType, TaskType >(
        cacheName, key, func, Operation::AFFINITY_CALL);
  }
  ...
  Future< void > AffinityRunAsync(const std::string& cacheName, const K& key,
                                  const F& action) {
    typedef ComputeJobHolderImpl< F, void > JobType;
    typedef SingleJobComputeTaskHolder< F, void > TaskType;

    return PerformAffinityTask< void, K, F, JobType, TaskType >(
        cacheName, key, action, Operation::AFFINITY_RUN);
  }
  ...
  Future< void > RunAsync(const F& action) {
    ...
    return PerformTask< void, F, JobType, TaskType >(Operation::UNICAST,
                                                     action);
  }
  ...
  Future< std::vector< R > > BroadcastAsync(const F& func) {
    ...
    return PerformTask< std::vector< R >, F, JobType, TaskType >(
        Operation::BROADCAST, func);
  }
  ...
  Future< void > BroadcastAsync(const F& func) {
    ...
    return PerformTask< void, F, JobType, TaskType >(Operation::BROADCAST,
                                                     func);
  }

  /**
   * Executes given Java task on the grid projection. If task for given name has
   * not been deployed yet, then 'taskName' will be used as task class name to
  ...
  /**
   * Asynchronously executes given Java task on the grid projection. If task for
   * given name has not been deployed yet, then 'taskName' will be used as task
   * class name to auto-deploy the task.
   *
   * @param taskName Java task name.
   * @param taskArg Argument of task execution of type A.
 ...
 private:
  ...
  template < typename T >
  struct FutureTypeForType {
    static const int32_t value = FutureType::F_OBJECT;
  };
  ...
  /**
   * Write Java task using provided writer. If task for given name has not been
   * deployed yet, then 'taskName' will be used as task class name to
   * auto-deploy the task.
   *
   * @param taskName Java task name.
   * @param taskArg Argument of task execution of type A.
  ...
  void WriteJavaTask(const std::string& taskName, const A* arg,
    ...
    if (!ProjectionContainsPredicate())
      ...
      typedef std::vector< documentdb::odbc::cluster::ClusterNode > ClusterNodes;
  ...
  Future< R > PerformJavaTaskAsync(const std::string& taskName, const A* arg) {
    ...
    std::shared_ptr< common::Cancelable > cancelable(
        new CancelableImpl(GetEnvironmentPointer(), target));
   ...
   */
  template < typename F >
  std::shared_ptr< common::Cancelable > PerformTask(Operation::Type operation,
                                                    int64_t jobHandle,
                                                    int64_t taskHandle,
                                                    const F& func) {
    common::concurrent::SharedPointer< interop::InteropMemory > mem =
  ...
  Future< R > PerformAffinityTask(const std::string& cacheName, const K& key,
                                  const F& func, Operation::Type opType) {
    enum { TYP_OBJ = 9 };
...
#endif  //_DOCUMENTDB_ODBC_IMPL_COMPUTE_COMPUTE_IMPL
