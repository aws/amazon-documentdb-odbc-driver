...
/**
 * @file
 * Declares documentdb::odbc::impl::compute::MultipleJobComputeTaskHolder class
 * template.
 */

#ifndef _DOCUMENTDB_ODBC_IMPL_COMPUTE_MULTIPLE_JOB_COMPUTE_TASK
#define _DOCUMENTDB_ODBC_IMPL_COMPUTE_MULTIPLE_JOB_COMPUTE_TASK

#include <stdint.h>
#include <vector>

#include <documentdb/odbc/common/promise.h>
#include <documentdb/odbc/impl/compute/compute_job_result.h>
...
class MultipleJobComputeTaskHolder : public ComputeTaskHolder {
 public:
   ...
   * @param reader Reader for stream with result.
   */
  virtual void JobResultError(const DocumentDbError& err) {
    ComputeJobResult< ResultType > res;
...
class MultipleJobComputeTaskHolder< F, void > : public ComputeTaskHolder {
 public:
   ...
   * @param handle Job handle.
   */
  MultipleJobComputeTaskHolder(int64_t handle) : ComputeTaskHolder(handle) {
    // No-op.
  }
...
#endif  //_DOCUMENTDB_ODBC_IMPL_COMPUTE_MULTIPLE_JOB_COMPUTE_TASK
