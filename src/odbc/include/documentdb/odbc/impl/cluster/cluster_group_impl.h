...
#ifndef _DOCUMENTDB_ODBC_IMPL_CLUSTER_CLUSTER_GROUP_IMPL
#define _DOCUMENTDB_ODBC_IMPL_CLUSTER_CLUSTER_GROUP_IMPL

#include <documentdb/odbc/common/concurrent.h>
#include <documentdb/odbc/jni/java.h>

#include <documentdb/odbc/ignite_predicate.h>
...
/* Shared pointer. */
typedef common::concurrent::SharedPointer< ClusterGroupImpl >
    SP_ClusterGroupImpl;
...
class IGNITE_FRIEND_EXPORT ClusterGroupImpl : private interop::InteropTarget {
  ...
  typedef common::concurrent::SharedPointer<
      ...
      SP_ClusterNodes;
  typedef common::concurrent::SharedPointer< ClusterNodePredicateHolder >
      SP_PredicateHolder;
 ...
 public:
  ...
  /**
   * Get cluster group for nodes containing given name and value specified in
   * user attributes.
   *
  ...
  /**
   * Get cluster group for all nodes that have cache with specified name, either
   * in client or server modes.
   *
   * @param cacheName Cache name.
   * @return Pointer to cluster group over nodes that have the cache with the
   * specified name running.
   */
  SP_ClusterGroupImpl ForCacheNodes(std::string cacheName);

  /**
   * Get cluster group for all client nodes that access cache with the specified
   * name.
   *
   * @param cacheName Cache name.
   * @return Pointer to cluster group over nodes that have the cache with the
  ...
  /**
   * Get cluster group for all data nodes that have the cache with the specified
   * name running.
   *
   * @param cacheName Cache name.
   * @return Pointer to cluster group over nodes that have the cache with the
  ...
  /**
   * Get cluster group consisting from the nodes in this cluster group residing
   * on the same host as the given node.
   *
   * @param node Cluster node.
   * @return Pointer to cluster group residing on the same host as the given
   * node.
   */
  SP_ClusterGroupImpl ForHost(documentdb::odbc::cluster::ClusterNode node);
   ...
   */
  SP_ClusterGroupImpl ForNode(documentdb::odbc::cluster::ClusterNode node);
   ...
   */
  SP_ClusterGroupImpl ForNodes(
      std::vector< documentdb::odbc::cluster::ClusterNode > nodes);

  /**
   * Get cluster group with one oldest node from the current cluster group.
   *
   * @return Pointer to cluster group with one oldest node from the current
   * cluster group.
   */
  SP_ClusterGroupImpl ForOldest();

  /**
   * Create a new cluster group which includes all nodes that pass the given
   * predicate filter.
   *
   * @param pred Pointer to predicate heap object.
   * @return Pointer to newly created cluster group.
   *
   * @throw DocumentDbError if there are no nodes in the cluster group.
   */
  SP_ClusterGroupImpl ForPredicate(
      IgnitePredicate< documentdb::odbc::cluster::ClusterNode >* pred);

  /**
   * Get cluster group with one random node from the current cluster group.
   *
   * @return Pointer to cluster group with one random node from the current
   * cluster group.
   */
  SP_ClusterGroupImpl ForRandom();

  /**
   * Get cluster group consisting from the nodes in this cluster group excluding
   * the local node.
   *
   * @return Pointer to cluster group consisting from the nodes in this cluster
   * group excluding the local node.
   */
   ...
   *
   * @return Pointer to cluster group with one youngest node in the current
   * cluster group.
   */
   ...
   * @return Local node.
   */
  documentdb::odbc::cluster::ClusterNode GetLocalNode();
   ...
   * @throw DocumentDbError if there are no nodes in the cluster group.
   */
  documentdb::odbc::cluster::ClusterNode GetNode();
   ...
   * @return Cluster node in this cluster group.
   *
   * @throw DocumentDbError if there is no node with specified ID.
   */
  documentdb::odbc::cluster::ClusterNode GetNode(Guid nid);
   ...
   */
  std::vector< documentdb::odbc::cluster::ClusterNode > GetNodes();
   ...
   */
  IgnitePredicate< documentdb::odbc::cluster::ClusterNode >* GetPredicate();
   ...
   * @return Pointer to predicate.
   */
  const IgnitePredicate< documentdb::odbc::cluster::ClusterNode >* GetPredicate()
      const;
   ...
   */
  std::vector< documentdb::odbc::cluster::ClusterNode > GetTopology(
      int64_t version);
 ...
 private:
   ...
   */
  std::vector< documentdb::odbc::cluster::ClusterNode > RefreshNodes();
...
#endif  //_DOCUMENTDB_ODBC_IMPL_CLUSTER_CLUSTER_GROUP_IMPL
