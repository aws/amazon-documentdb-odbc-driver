...
#ifndef _DOCUMENTDB_ODBC_IMPL_BINARY_BINARY_READER
#define _DOCUMENTDB_ODBC_IMPL_BINARY_BINARY_READER
...
#include "documentdb/odbc/binary/binary_enum.h"
...
class DOCUMENTDB_IMPORT_EXPORT BinaryReaderImpl {
 public:
  ...
  BinaryReaderImpl(interop::InteropInputStream* stream,
                   BinaryIdResolver* idRslvr, int32_t pos, bool usrType,
                   int32_t typeId, int32_t hashCode, int32_t len,
                   int32_t rawOff, int32_t footerBegin, int32_t footerEnd,
                   BinaryOffsetType::Type schemaType);

  /**
   * Constructor used to construct light-weight reader allowing only raw
   * operations and read of primitives.
   *
   * @param stream Interop stream.
   */
  BinaryReaderImpl(interop::InteropInputStream* stream);
   ...
   * length. -1 will be returned in case array in stream was null.
   */
  int32_t ReadInt16Array(const char* fieldName, int16_t* res,
                         const int32_t len);
   ...
   * length. -1 will be returned in case array in stream was null.
   */
  int32_t ReadUInt16Array(const char* fieldName, uint16_t* res,
                          const int32_t len);
   ...
   * length. -1 will be returned in case array in stream was null.
   */
  int32_t ReadInt32Array(const char* fieldName, int32_t* res,
                         const int32_t len);
   ...
   * length. -1 will be returned in case array in stream was null.
   */
  int32_t ReadInt64Array(const char* fieldName, int64_t* res,
                         const int32_t len);
   ...
   * length. -1 will be returned in case array in stream was null.
   */
  int32_t ReadDoubleArray(const char* fieldName, double* res,
                          const int32_t len);
   ...
   * length. -1 will be returned in case array in stream was null.
   */
  int32_t ReadTimestampArray(const char* fieldName, Timestamp* res,
                             const int32_t len);
   ...
   * @return Enum entry.
   */
  documentdb::odbc::binary::BinaryEnumEntry ReadBinaryEnum();
   ...
   * @return Enum entry.
   */
  documentdb::odbc::binary::BinaryEnumEntry ReadBinaryEnum(const char* fieldName);

  /**
   * Read string.
   *
   * @param len Expected length of string.
   * @param res Array to store data to (should be able to accomodate
   * null-terminator).
   * @return Actual amount of elements read. If "len" argument is less than
   * actual array size or resulting array is set to null, nothing will be
   * written to resulting array and returned value will contain required array
   ...
   *
   * @param fieldName Field name.
   * @param res Array to store data to (should be able to acocmodate
   * null-terminator).
   * @param len Expected length of string.
   * @return Actual amount of elements read. If "len" argument is less than
   * actual array size or resulting array is set to null, nothing will be
   ...
   * @return Read session ID.
   */
  int32_t ReadCollection(documentdb::odbc::binary::CollectionType::Type* typ,
                         int32_t* size);
   ...
   * @return Read session ID.
   */
  int32_t ReadMap(documentdb::odbc::binary::MapType::Type* typ, int32_t* size);
  ...
  int32_t ReadMap(const char* fieldName,
                  documentdb::odbc::binary::MapType::Type* typ, int32_t* size);
   ...
   */
  documentdb::odbc::binary::CollectionType::Type ReadCollectionType();
  ...
  template < typename T >
  T ReadEnum() {
    documentdb::odbc::binary::BinaryEnumEntry entry = ReadBinaryEnum();
  ...
  T ReadEnum(const char* fieldName) {
    documentdb::odbc::binary::BinaryEnumEntry entry = ReadBinaryEnum(fieldName);
    ...
    switch (hdr) {
      ...
      case DOCUMENTDB_TYPE_BINARY: {
        ...
        stream->Position(curPos
                         + portOff);  // Position stream right on the object.

        ReadTopObject0< R, T >(res);

        stream->Position(curPos + portLen
                         + 4);  // Position stream after binary.
      ...
      case DOCUMENTDB_HDR_FULL: {
        ...
        BinaryReaderImpl readerImpl(stream, &idRslvr, pos, usrType, typeId,
                                    hashCode, len, rawOff, footerBegin,
  ...
  T GetNull() const {
    ...
    documentdb::odbc::binary::BinaryType< T >::GetNull(res);
 ...
 private:
   ...
   */
  template < typename T >
  T DeserializeEnumEntry(documentdb::odbc::binary::BinaryEnumEntry entry) {
    typedef documentdb::odbc::binary::BinaryEnum< T > TypeMeta;
   ...
   * @param len Length.
   */
  static void ReadGuidArrayInternal(interop::InteropInputStream* stream,
   ...
   * @param len Length.
   */
  static void ReadDateArrayInternal(interop::InteropInputStream* stream,
   ...
   * @param len Length.
   */
  static void ReadTimestampArrayInternal(interop::InteropInputStream* stream,
   ...
   * @param len Length.
   */
  static void ReadTimeArrayInternal(interop::InteropInputStream* stream,
   ...
   */
  template < typename T >
  T Read(const char* fieldName, T (*func)(interop::InteropInputStream*),
         const int8_t expHdr, T dflt) {
    CheckRawMode(false);
    CheckSingleMode(true);
   ...
   */
  template < typename T >
  int32_t ReadRawArray(T* res, const int32_t len,
                       void (*func)(interop::InteropInputStream*, T* const,
                                    const int32_t),
                       const int8_t expHdr) {
    {
      CheckRawMode(true);
   ...
   */
  template < typename T >
  int32_t ReadArray(const char* fieldName, T* res, const int32_t len,
                    void (*func)(interop::InteropInputStream*, T* const,
                                 const int32_t),
                    const int8_t expHdr) {
    {
   ...
   */
  template < typename T >
  static int32_t ReadArrayInternal(T* res, const int32_t len,
                                   interop::InteropInputStream* stream,
                                   void (*func)(interop::InteropInputStream*,
                                                T*, int32_t),
                                   const int8_t expHdr) {
    int8_t hdr = stream->ReadInt8();
  ...
  static void ReadArrayToVectorInternal(
      std::vector< T >& res, interop::InteropInputStream* stream,
   ...
   */
  template < typename T >
  static T ReadNullable(interop::InteropInputStream* stream,
   ...
   * @return Session ID.
   */
  int32_t StartContainerSession(const bool expRawMode, const int8_t expHdr,
                                int32_t* size);
   ...
   * @return Enum entry.
   */
  documentdb::odbc::binary::BinaryEnumEntry ReadBinaryEnumInternal();
   ...
   * @return Collection type.
   */
  documentdb::odbc::binary::CollectionType::Type ReadCollectionTypeUnprotected();
  ...
  T ReadTopObject0(const int8_t expHdr,
                   T (*func)(documentdb::odbc::impl::interop::InteropInputStream*));
};
...
#endif  //_DOCUMENTDB_ODBC_IMPL_BINARY_BINARY_READER
