...
/**
 * @file
 * Declares documentdb::odbc::Ignite class.
 */

#ifndef _DOCUMENTDB_ODBC_IGNITE
#define _DOCUMENTDB_ODBC_IGNITE

#include <documentdb/odbc/impl/ignite_impl.h>

#include <documentdb/odbc/ignite_configuration.h>
#include <documentdb/odbc/cache/cache.h>
#include <documentdb/odbc/cache/cache_affinity.h>
#include <documentdb/odbc/compute/compute.h>
#include <documentdb/odbc/cluster/ignite_cluster.h>
...
class IGNITE_IMPORT_EXPORT Ignite {
 ...
 public:
   ...
   */
  template < typename K, typename V >
  cache::Cache< K, V > GetCache(const char* name, DocumentDbError& err) {
   ...
   */
  template < typename K, typename V >
  cache::Cache< K, V > GetOrCreateCache(const char* name, DocumentDbError& err) {
   ...
   */
  template < typename K, typename V >
  cache::Cache< K, V > CreateCache(const char* name, DocumentDbError& err) {
 ...
 private:
  ...
  documentdb::odbc::common::concurrent::SharedPointer< impl::IgniteImpl > impl;
};
}  // namespace odbc
}  // namespace documentdb

#endif  //_DOCUMENTDB_ODBC_IGNITE
