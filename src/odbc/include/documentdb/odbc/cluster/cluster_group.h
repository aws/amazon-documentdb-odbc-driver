...
/**
 * @file
 * Declares documentdb::odbc::cluster::ClusterGroup class.
 */

#ifndef _DOCUMENTDB_ODBC_CLUSTER_CLUSTER_GROUP
#define _DOCUMENTDB_ODBC_CLUSTER_CLUSTER_GROUP
 ...
 * Cluster group allows to group cluster nodes into various subgroups to perform
 * distributed operations on them. The IgniteCluster interface itself also
 * contains the ClusterGroup which makes an instance of IgniteCluster into a
...
class DOCUMENTDB_IMPORT_EXPORT ClusterGroup {
 ...
 public:
  ...
  /**
   * Get ClusterGroup for all data nodes that have the cache with the specified
   * name running.
   *
   * @param cacheName Cache name.
   * @return Cluster group over nodes that have the cache with the specified
   ...
   * predicate filter.
   *
   * @param pred Pointer to predicate heap object. User should NOT free the
...
#endif  //_DOCUMENTDB_ODBC_CLUSTER_CLUSTER_GROUP
