...
#ifndef _DOCUMENTDB_ODBC_COMMON_TYPES
#define _DOCUMENTDB_ODBC_COMMON_TYPES
...
struct SqlState {
  enum Type {
    ...
    /** The timeout period expired before the data source responded to the
       request. */
...
struct ResponseStatus {
  enum Type {
    ...
    /** General parsing error - for the cases when there's no more specific code
       available. */
    ...
    /** Code encountered SQL statement of some type that it did not expect in
       current analysis context. */
    UNEXPECTED_OPERATION = 2001,

    /** Code encountered SQL expression of some type that it did not expect in
       current analysis context. */
    UNEXPECTED_ELEMENT_TYPE = 2002,

    /** Analysis detected that the statement is trying to directly UPDATE key or
       its fields. */
    KEY_UPDATE = 2003,
    ...
    /** Attempt to UPDATE or DELETE a key whose value has been updated
...
/**
 * Convert environment attribute to internal EnvironmentAttribute::Type type
 * value.
 *
...
#endif  //_DOCUMENTDB_ODBC_COMMON_TYPES
