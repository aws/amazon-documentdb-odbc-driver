...
/**
 * @file
 * Declares documentdb::odbc::Reference class.
 */

#ifndef _DOCUMENTDB_ODBC_COMMON_REFERENCE
#define _DOCUMENTDB_ODBC_COMMON_REFERENCE

#include <cstddef>

#include <documentdb/odbc/common/common.h>
#include <documentdb/odbc/common/concurrent.h>
#include <documentdb/odbc/common/reference_impl.h>
...
class ConstReference {
 ...
 public:
   ...
   */
  explicit ConstReference(common::ConstReferenceImplBase* ptr,
                          ptrdiff_t offset = 0)
  ...
  ConstReference(const ConstReference& other)
      : ptr(other.ptr), offset(other.offset) {
    // No-op.
  }
  ...
  ConstReference(const ConstReference< T2 >& other)
    ...
    ptrdiff_t diff =
        reinterpret_cast< ptrdiff_t >(p1) - reinterpret_cast< ptrdiff_t >(p0);
    offset += diff;
  }
  ...
  const T* Get() const {
    return reinterpret_cast< const T* >(
        reinterpret_cast< ptrdiff_t >(ptr.Get()->Get()) + offset);
  }
...
class Reference {
 ...
 public:
   ...
   * @param other Another instance.
   */
  Reference(const Reference& other) : ptr(other.ptr), offset(other.offset) {
    // No-op.
  }
...
/**
 * Make documentdb::odbc::Reference instance out of smart pointer.
 *
...
Reference< typename T::element_type > MakeReferenceFromSmartPointer(T ptr) {
  common::ReferenceSmartPointer< T >* impl =
...
/**
 * Make documentdb::ConstReference instance out of smart pointer.
 *
 * Template type 'T' should be a smart pointer and provide pointer semantics:
 * - There should be defined type 'T::element_type', showing underlying type.
 * - Type 'T' should be dereferencible (should have operators
 ...
 */
template < typename T >
ConstReference< typename T::element_type > MakeConstReferenceFromSmartPointer(
    T ptr) {
  common::ReferenceSmartPointer< T >* impl =
      new common::ReferenceSmartPointer< T >();
...
/**
 * Copy object and wrap it to make documentdb::odbc::Reference instance.
 *
...
Reference< T > MakeReferenceFromCopy(const T& val) {
  common::ReferenceOwningRawPointer< T >* impl =
      new common::ReferenceOwningRawPointer< T >(new T(val));

  return Reference< T >(impl);
}

/**
 * Copy object and wrap it to make documentdb::ConstReference instance.
 *
 * Template type 'T' should be copy-constructible.
 *
 * @param val Instance.
...
/**
 * Make documentdb::odbc::Reference instance out of pointer and pass its ownership.
...
/**
 * Make documentdb::ConstReference instance out of pointer and pass its ownership.
 * Passed object deleted by Ignite when no longer needed.
 *
 * There are no requirements for the template type T.
 *
...
/**
 * Make documentdb::odbc::Reference instance out of reference.
...
Reference< T > MakeReference(T& val) {
  common::ReferenceNonOwningRawPointer< T >* impl =
...
/**
 * Make documentdb::odbc::Reference instance out of pointer.
 * Ignite do not manage passed object and does not affect its lifetime.
 *
 * There are no requirements for the template type T.
 *
...
/**
 * Make documentdb::ConstReference instance out of constant reference.
 * Ignite do not manage passed object and does not affect its lifetime.
 *
 * There are no requirements for the template type T.
 *
...
ConstReference< T > MakeConstReference(const T& val) {
  common::ConstReferenceNonOwningRawPointer< T >* impl =
...
/**
 * Make documentdb::ConstReference instance out of constant pointer.
 * Ignite do not manage passed object and does not affect its lifetime.
 *
 * There are no requirements for the template type T.
 *
...
#endif  //_DOCUMENTDB_ODBC_COMMON_REFERENCE
