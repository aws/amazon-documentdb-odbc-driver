...
/**
 * @file
 * Declares documentdb::odbc::compute::Compute class.
 */

#ifndef _DOCUMENTDB_ODBC_COMPUTE_COMPUTE
#define _DOCUMENTDB_ODBC_COMPUTE_COMPUTE

#include <documentdb/odbc/common/common.h>

#include <documentdb/odbc/documentdb_error.h>
#include <documentdb/odbc/future.h>
#include <documentdb/odbc/compute/compute_func.h>
...
class IGNITE_IMPORT_EXPORT Compute {
 public:
  ...
  R AffinityCall(const std::string& cacheName, const K& key, const F& func) {
    return impl.Get()
        ->AffinityCallAsync< R, K, F >(cacheName, key, func)
        .GetValue();
  }
  ...
  void AffinityRun(const std::string& cacheName, const K& key,
                   const F& action) {
    return impl.Get()
        ->AffinityRunAsync< K, F >(cacheName, key, action)
        .GetValue();
  }
...
#endif  //_DOCUMENTDB_ODBC_COMPUTE_COMPUTE
