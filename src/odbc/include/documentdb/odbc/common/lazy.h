 ...
 */
#ifndef _DOCUMENTDB_ODBC_COMMON_LAZY
#define _DOCUMENTDB_ODBC_COMMON_LAZY
...
class DOCUMENTDB_IMPORT_EXPORT Lazy {
  ...
  struct InitFunctionType0 : InitFunctionType {
     ...
     * @param func Function.
     */
    InitFunctionType0(F func) : func(func) {
      // No-op.
    }
 ...
 public:
  ...
  template < typename F >
  Lazy(F initFunc)
      : initFunc(new InitFunctionType0< F >(initFunc)), instance(), lock() {
    // No-op.
  }
  ...
  void Init(F initFunc) {
    this->initFunc = concurrent::SharedPointer< InitFunctionType >(
        new InitFunctionType0< F >(initFunc));
  }
  ...
  concurrent::SharedPointer< InstanceType > Get() {
    ...
    instance =
        concurrent::SharedPointer< InstanceType >(initFunc.Get()->Init());
...
#endif  // _DOCUMENTDB_ODBC_COMMON_LAZY
