...
/**
 * @file
 * Declares documentdb::odbc::cache::MutableCacheEntry class template.
 */

#ifndef _DOCUMENTDB_ODBC_CACHE_MUTABLE_CACHE_ENTRY
#define _DOCUMENTDB_ODBC_CACHE_MUTABLE_CACHE_ENTRY
...
class MutableCacheEntry {
 public:
   ...
   * Default constructor.
   */
  MutableCacheEntry() : key(), val(), exists(false) {
    // No-op.
  }
  ...
  MutableCacheEntry(const MutableCacheEntry& other)
      : key(other.key), val(other.val), exists(other.exists) {
    // No-op.
  }
...
#endif  //_DOCUMENTDB_ODBC_CACHE_MUTABLE_CACHE_ENTRY
