...
/**
 * @file
 * Declares documentdb::odbc::cache::query::SqlFieldsQuery class.
 */

#ifndef _DOCUMENTDB_ODBC_CACHE_QUERY_QUERY_SQL_FIELDS
#define _DOCUMENTDB_ODBC_CACHE_QUERY_QUERY_SQL_FIELDS

#include <stdint.h>
#include <string>
#include <vector>

#include <documentdb/odbc/impl/cache/query/query_argument.h>
...
class SqlFieldsQuery {
 public:
  ...
  SqlFieldsQuery(const SqlFieldsQuery& other)
    ...
    typedef std::vector<
        impl::cache::query::QueryArgumentBase* >::const_iterator Iter;
   ...
   * excessive GC pauses and even OutOfMemoryError. Use this flag as a hint for
   * Ignite to fetch result set lazily, thus minimizing memory consumption at
   * the cost of moderate performance hit.
   *
   * Defaults to @c false, meaning that the whole result set is fetched to
   * memory eagerly.
   *
   ...
   *
   * If set to true query optimizer will not reorder tables in join. By default
   * is false.
   *
   * It is not recommended to enable this property unless you are sure that your
   * indexes and the query itself are correct and tuned as much as possible but
   ...
   *
   * When disabled, join results will only contain colocated data (joins work
   * locally). When enabled, joins work as expected, no matter how the data is
   * distributed.
   *
   ...
   * Add argument.
   *
   * Template argument type should be copy-constructable and assignable. Also
   * BinaryType class template should be specialized for this type.
   *
   * @param arg Argument.
   */
  template < typename T >
   ...
   * If not set, current cache name is used, which means you can omit schema
   * name for tables within the current cache.
   *
...
#endif  //_DOCUMENTDB_ODBC_CACHE_QUERY_QUERY_SQL_FIELDS
