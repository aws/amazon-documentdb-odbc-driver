...
/**
 * @file
 * Declares documentdb::odbc::cache::CacheAffinity class.
 */

#ifndef _DOCUMENTDB_ODBC_CACHE_AFFINITY
#define _DOCUMENTDB_ODBC_CACHE_AFFINITY
 ...
 * are backups for a partitioned or replicated cache. You can get an instance of
 * this interface by calling Ignite.GetAffinity(cacheName) method.
 *
...
class DOCUMENTDB_IMPORT_EXPORT CacheAffinity {
 public:
   ...
   */
  CacheAffinity(impl::cache::SP_CacheAffinityImpl impl) : impl(impl) {
    // No-op.
  }
   ...
   * this cache.
   */
  std::map< cluster::ClusterNode, std::vector< K > > MapKeysToNodes(
   ...
   */
  std::map< int32_t, cluster::ClusterNode > MapPartitionsToNodes(
   ...
   * node always first.
   */
  std::vector< cluster::ClusterNode > MapPartitionToPrimaryAndBackups(
      int32_t part) {
...
#endif  //_DOCUMENTDB_ODBC_CACHE_AFFINITY
