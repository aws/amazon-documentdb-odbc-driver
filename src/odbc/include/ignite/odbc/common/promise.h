...
/**
 * @file
 * Declares ignite::odbc::commom::Promise class template.
 */
...
#include <ignite/odbc/common/shared_state.h>
...
class Promise {
 public:
  ...
  ~Promise() {
    ...
    if (!state0->IsSet())
      state0->SetError(IgniteError(
          IgniteError::IGNITE_ERR_FUTURE_STATE,
   ...
   * Set value.
   *
   * @throw IgniteError with IgniteError::IGNITE_ERR_FUTURE_STATE if error or
