...
#ifndef _IGNITE_ODBC_IMPL_OPERATIONS
#define _IGNITE_ODBC_IMPL_OPERATIONS
...
#include "ignite/odbc/impl/helpers.h"
...
class InputOperation {
 public:
  ...
  virtual void ProcessInput(
      ignite::odbc::impl::binary::BinaryWriterImpl& writer) = 0;
};
...
class In1Operation : public InputOperation {
 public:
  ...
  virtual void ProcessInput(
      ignite::odbc::impl::binary::BinaryWriterImpl& writer) {
...
class In3Operation : public InputOperation {
 public:
   ...
   */
  In3Operation(const T1& val1, const T2& val2, const T3& val3)
...
class InSetOperation : public InputOperation {
 public:
  ...
  virtual void ProcessInput(
    ...
    for (typename std::set< T >::const_iterator it = val.begin();
         it != val.end(); ++it)
...
class InCacheLocalPeekOperation : public InputOperation {
 public:
   ...
   */
  InCacheLocalPeekOperation(const T& key, int32_t peekModes)
      : key(key), peekModes(peekModes) {
    // No-op.
  }
...
class InCacheInvokeOperation : public InputOperation {
 public:
   ...
   * @param val2 Second value.
   */
  InCacheInvokeOperation(const T1& val1, const T2& val2)
      : val1(val1), val2(val2) {
    // No-op.
  }
...
class InIterOperation : public InputOperation {
 public:
   ...
   */
  InIterOperation(Iter begin, Iter end) : begin(begin), end(end) {
    // No-op.
  }
...
class Out2Operation : public OutputOperation {
 public:
   ...
   * @param val2 Value 2.
   */
  Out2Operation(T1& val1, T2& val2) : val1(val1), val2(val2) {
    // No-op.
  }
...
class OutMapOperation : public OutputOperation {
 public:
   ...
   * @param val Value.
   */
  OutMapOperation(std::map< T1, T2 >& val) : val(val) {
    // No-op.
  }
...
class OutQueryGetAllOperation : public OutputOperation {
 public:
   ...
   * Constructor.
   */
  OutQueryGetAllOperation(
      std::vector< ignite::odbc::cache::CacheEntry< K, V > >& res)
      : res(res) {
    // No-op.
  }
  ...
  virtual void ProcessOutput(binary::BinaryReaderImpl& reader) {
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      res.push_back(ignite::odbc::cache::CacheEntry< K, V >(key, val));
    }
  }
 ...
 */
template < typename K, typename V, typename Iter,
           typename Pair = ignite::odbc::cache::CacheEntry< K, V > >
...
#endif  //_IGNITE_ODBC_IMPL_OPERATIONS
