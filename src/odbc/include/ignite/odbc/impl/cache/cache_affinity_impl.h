        ...
        namespace impl
        {
            namespace cache
            {
                ...
                public:
                     ...
                     */
                    template <typename K>
                    bool IsPrimaryOrBackup(ignite::odbc::cluster::ClusterNode node, const K& key)
                    {
                        common::concurrent::SharedPointer<interop::InteropMemory> memIn = GetEnvironment().AllocateMemory();
                        interop::InteropOutputStream out(memIn.Get());
                        binary::BinaryWriterImpl writer(&out, GetEnvironment().GetTypeManager());
                     ...
                     */
                    std::vector<int32_t> GetPrimaryPartitions(ignite::odbc::cluster::ClusterNode node);
                     ...
                     */
                    std::vector<int32_t> GetBackupPartitions(ignite::odbc::cluster::ClusterNode node);
                     ...
                     */
                    std::vector<int32_t> GetAllPartitions(ignite::odbc::cluster::ClusterNode node);
                    ...
                    std::map<ignite::odbc::cluster::ClusterNode, std::vector<TK> > MapKeysToNodes(const std::vector<TK>& keys)
                    {
                        common::concurrent::SharedPointer<interop::InteropMemory> memIn = GetEnvironment().AllocateMemory();
                        ...
                        for (int32_t i = 0; i < cnt; i++)
                        {
                            ...
                            ignite::odbc::cluster::ClusterNode key(GetEnvironment().GetNode(reader.ReadGuid()));
                            reader.ReadCollection<TK>(std::back_inserter(val));
    
                            ret.insert(std::pair<ignite::odbc::cluster::ClusterNode, std::vector<TK> >(key, val));
                        }
                     ...
                     */
                    template <typename TK>
                    std::vector<ignite::odbc::cluster::ClusterNode> MapKeyToPrimaryAndBackups(const TK& key)
                    {
                        common::concurrent::SharedPointer<interop::InteropMemory> memIn = GetEnvironment().AllocateMemory();
                        common::concurrent::SharedPointer<interop::InteropMemory> memOut = GetEnvironment().AllocateMemory();
                        interop::InteropOutputStream out(memIn.Get());
                     ...
                     */
                    std::map<int32_t, ignite::odbc::cluster::ClusterNode> MapPartitionsToNodes(const std::vector<int32_t>& parts);
                     ...
                     */
                    std::vector<ignite::odbc::cluster::ClusterNode> MapPartitionToPrimaryAndBackups(int32_t part);
                ...
                private:
                     ...
                     */
                    std::vector<int32_t> GetPartitions(int32_t opType, ignite::odbc::cluster::ClusterNode node);
                };
            }
        }
    }
