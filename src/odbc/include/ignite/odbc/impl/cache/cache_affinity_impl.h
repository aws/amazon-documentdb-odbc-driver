...
#ifndef _IGNITE_ODBC_CACHE_AFFINITY_IMPL
#define _IGNITE_ODBC_CACHE_AFFINITY_IMPL
...
class IGNITE_FRIEND_EXPORT CacheAffinityImpl : private interop::InteropTarget {
 ...
 public:
   ...
   */
  template < typename K >
  bool IsPrimary(ignite::odbc::cluster::ClusterNode node, const K& key) {
   ...
   */
  template < typename K >
  bool IsBackup(ignite::odbc::cluster::ClusterNode node, const K& key) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
        GetEnvironment().AllocateMemory();
    interop::InteropOutputStream out(memIn.Get());
    binary::BinaryWriterImpl writer(&out, GetEnvironment().GetTypeManager());
   ...
   */
  template < typename K >
  bool IsPrimaryOrBackup(ignite::odbc::cluster::ClusterNode node,
                         const K& key) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
        GetEnvironment().AllocateMemory();
    interop::InteropOutputStream out(memIn.Get());
  ...
  std::vector< int32_t > GetPrimaryPartitions(
      ignite::odbc::cluster::ClusterNode node);
  ...
  std::map< ignite::odbc::cluster::ClusterNode, std::vector< TK > >
  MapKeysToNodes(const std::vector< TK >& keys) {
    ...
    std::map< ignite::odbc::cluster::ClusterNode, std::vector< TK > > ret;

    int32_t cnt = reader.ReadInt32();
    for (int32_t i = 0; i < cnt; i++) {
      std::vector< TK > val;

      ignite::odbc::cluster::ClusterNode key(
          GetEnvironment().GetNode(reader.ReadGuid()));
      reader.ReadCollection< TK >(std::back_inserter(val));

      ret.insert(
          std::pair< ignite::odbc::cluster::ClusterNode, std::vector< TK > >(
              key, val));
    }
   ...
   */
  template < typename TK >
  ignite::odbc::cluster::ClusterNode MapKeyToNode(const TK& key) {
    Guid nodeId;
    In1Operation< TK > inOp(key);
   ...
   */
  template < typename TK >
  std::vector< ignite::odbc::cluster::ClusterNode > MapKeyToPrimaryAndBackups(
      const TK& key) {
    common::concurrent::SharedPointer< interop::InteropMemory > memIn =
        GetEnvironment().AllocateMemory();
    common::concurrent::SharedPointer< interop::InteropMemory > memOut =
    ...
    std::vector< ignite::odbc::cluster::ClusterNode > ret;
    int32_t cnt = reader.ReadInt32();
    ret.reserve(cnt);
    for (int32_t i = 0; i < cnt; i++)
   ...
   */
  ignite::odbc::cluster::ClusterNode MapPartitionToNode(int32_t part);
   ...
   */
  std::map< int32_t, ignite::odbc::cluster::ClusterNode > MapPartitionsToNodes(
   ...
   * node always first.
   */
  std::vector< ignite::odbc::cluster::ClusterNode >
...
#endif  // _IGNITE_ODBC_CACHE_AFFINITY_IMPL
