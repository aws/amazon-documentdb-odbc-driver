...
/**
 * @file
 * Declares ignite::odbc::impl::cache::query::continuous::ContinuousQueryImpl
 * class.
 */

#ifndef _IGNITE_ODBC_IMPL_CACHE_QUERY_CONTINUOUS_CONTINUOUS_QUERY_IMPL
#define _IGNITE_ODBC_IMPL_CACHE_QUERY_CONTINUOUS_CONTINUOUS_QUERY_IMPL
...
#include <ignite/odbc/cache/event/cache_entry_event_listener.h>
#include <ignite/odbc/binary/binary_raw_reader.h>
#include <ignite/odbc/impl/cache/event/cache_entry_event_filter_holder.h>
 ...
 * ignite::odbc::cache::Cache::QueryContinuous().
 */
...
class ContinuousQueryImpl : public ContinuousQueryImplBase {
 public:
   ...
   */
  ContinuousQueryImpl(
      Reference< ignite::odbc::cache::event::CacheEntryEventListener< K, V > >&
          lsnr,
      bool loc)
   ...
   */
  const ignite::odbc::cache::event::CacheEntryEventListener< K, V >&
  GetListener() const {
    return lsnr.Get();
  }
   ...
   * @return Cache entry event listener.
   */
  ignite::odbc::cache::event::CacheEntryEventListener< K, V >& GetListener() {
    return lsnr.Get();
  }
   ...
   */
  virtual void ReadAndProcessEvents(
      ignite::odbc::binary::BinaryRawReader& reader) {
    // Number of events.
    int32_t cnt = reader.ReadInt32();

    // Storing events here.
    std::vector< ignite::odbc::cache::CacheEntryEvent< K, V > > events;
    events.resize(cnt);
 ...
 private:
  ...
  Reference< ignite::odbc::cache::event::CacheEntryEventListener< K, V > > lsnr;
};
...
class RemoteFilterHolder : public ContinuousQueryImplBase {
 public:
  ...
  virtual void ReadAndProcessEvents(ignite::odbc::binary::BinaryRawReader&) {
    throw IgniteError(
...
#endif  //_IGNITE_ODBC_IMPL_CACHE_QUERY_CONTINUOUS_CONTINUOUS_QUERY_IMPL
