...
BOOST_FIXTURE_TEST_SUITE(SqlTypesTestSuite, documentdb::SqlTestSuiteFixture)
...
BOOST_AUTO_TEST_CASE(TestByteArrayParam) {
  ...
  ret = SQLBindParameter(stmt, 1, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_VARBINARY,
                         paramData.size(), 0, &paramData[0], paramData.size(),
                         &paramLen);
...
BOOST_AUTO_TEST_CASE(TestByteArrayParamInsert) {
  ...
  ret = SQLBindParameter(stmt, 2, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_VARBINARY,
                         paramData.size(), 0, &paramData[0], paramData.size(),
                         &paramLen);
  ...
  BOOST_REQUIRE_EQUAL_COLLECTIONS(out.i8ArrayField.begin(),
                                  out.i8ArrayField.end(), paramData.begin(),
                                  paramData.end());
}
...
BOOST_AUTO_TEST_CASE(TestByteParamInsert) {
  ...
  int8_t data = 2;
  ret = SQLBindParameter(stmt, 2, SQL_PARAM_INPUT, SQL_C_TINYINT, SQL_TINYINT,
                         0, 0, &data, 0, 0);
...
BOOST_AUTO_TEST_CASE(TestTimestampInsert) {
  ...
  ret =
      SQLBindParameter(stmt, 2, SQL_PARAM_INPUT, SQL_C_TIMESTAMP, SQL_TIMESTAMP,
                       sizeof(data), 0, &data, sizeof(data), &lenInd);
  ...
  BOOST_REQUIRE_EQUAL(out.timestampField.GetSecondFraction(),
...
BOOST_AUTO_TEST_CASE(TestTimeInsert) {
  ...
  SQLLEN lenInd = sizeof(data);
  ret = SQLBindParameter(stmt, 2, SQL_PARAM_INPUT, SQL_C_TIME, SQL_TIME,
                         sizeof(data), 0, &data, sizeof(data), &lenInd);
