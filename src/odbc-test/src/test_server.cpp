...
TestServerSession::TestServerSession(
    ...
    : socket(service), responses(responses), requestsResponded(0) {
  // No-op.
}
...
void TestServerSession::HandleRequestSizeReceived(
...
void TestServerSession::HandleRequestReceived(
    const boost::system::error_code& error, size_t bytesTransferred) {
...
void TestServerSession::HandleResponseSent(
    const boost::system::error_code& error, size_t bytesTransferred) {
...
void TestServer::Start() {
  if (!serverThread) {
    StartAccept();
    serverThread.reset(new boost::thread(
...
void TestServer::StartAccept() {
  ...
  acceptor.async_accept(newSession->GetSocket(),
                        boost::bind(&TestServer::HandleAccept, this, newSession,
                                    placeholders::error));
}

void TestServer::HandleAccept(boost::shared_ptr< TestServerSession > session,
