...
BOOST_AUTO_TEST_CASE(TestCast) {
  ...
  CheckSingleResult< SQLINTEGER >(
      "SELECT CAST(strField AS INT) + i32Field FROM TestType",
      common::LexicalCast< int32_t >(in.strField) + in.i32Field);

  CheckSingleResult< std::string >(
      "SELECT CAST(i32Field AS VARCHAR) || strField FROM TestType",
...
BOOST_AUTO_TEST_CASE(TestCoalesce) {
  ...
  CheckSingleResult< std::string >("SELECT COALESCE(NULL, NULL, 'Three')",
                                   "Three");
}
...
BOOST_AUTO_TEST_CASE(TestNullif) {
  ...
  CheckSingleResult< std::string >(
      "SELECT NULLIF(strField, 'blablabla') FROM TestType", in.strField);
  CheckSingleResult< std::string >(
      "SELECT NULLIF(strField, 'SomeValue') FROM TestType", "");
}
