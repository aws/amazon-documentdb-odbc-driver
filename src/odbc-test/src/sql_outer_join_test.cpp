...
BOOST_AUTO_TEST_CASE(TestOuterJoinLeft) {
  ...
  SQLRETURN ret =
      SQLBindCol(stmt, 1, SQL_C_SLONG, &columns[0], 0, &columnsLen[0]);
  ...
  SQLCHAR request[] =
      "SELECT T1.i32Field, T2.i16Field FROM "
      "{oj TestType T1 LEFT OUTER JOIN TestType T2 ON T1.i32Field = "
      "T2.i16Field}";
...
BOOST_AUTO_TEST_CASE(TestOuterJoinOrdering) {
  ...
  SQLCHAR request[] =
      "SELECT T1.i32Field, T2.i16Field FROM "
      "{oj TestType T1 LEFT OUTER JOIN TestType T2 ON T2.i16Field = "
      "T1.i32Field}";
