...
struct TransactionTestSuiteFixture : public odbc::OdbcTestSuite {
   ...
   * Constructor.
   */
  TransactionTestSuiteFixture() : grid(StartAdditionalNode("NodeMain")) {
    // No-op.
  }
   ...
   * @param value Value.
   */
  static void InsertTestValue(SQLHSTMT stmt, int64_t key,
   ...
   * @param value Value.
   */
  static void UpdateTestValue(SQLHSTMT stmt, int64_t key,
                              const std::string& value) {
   ...
   * @param expect Expected value.
   */
  static void CheckTestValue(SQLHSTMT stmt, int64_t key,
    ...
    SQLRETURN ret = SQLBindCol(stmt, 1, SQL_C_CHAR, &strField, sizeof(strField),
                               &strFieldLen);
...
BOOST_AUTO_TEST_CASE(TransactionVersionMismatchError) {
  ...
  try {
    ...
    BOOST_CHECK(
        err.message.find("Cannot serialize transaction due to write conflict")
        != err.message.npos);
  ...
  try {
    ...
    BOOST_CHECK(err.message.find("Transaction is already completed")
                != err.message.npos);
