 ...
 * 2. Establish connection using UID and PWD arguments. Check that it
...
/**
 * Check that connection with UID, USER, PWD and PASSWORD arguments established
 * successfully.
 *
 * 1. Start test node with configured authentication.
 * 2. Establish connection using UID, USER, PWD and PASSWORD arguments. Check
 * that it established successfully.
 * 3. Check that connection returns warning that password and user arguments
 * duplicated.
...
BOOST_AUTO_TEST_CASE(TestConnectionBothAuthSuccess) {
  ...
  BOOST_CHECK(
      message.find("Re-writing PASSWORD (have you specified it several times?")
      != std::string::npos);
  ...
  BOOST_CHECK(message.find("01S02") != std::string::npos);
  BOOST_CHECK(
      message.find("Re-writing USER (have you specified it several times?")
      != std::string::npos);
