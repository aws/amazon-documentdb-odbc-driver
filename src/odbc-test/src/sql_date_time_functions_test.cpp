...
BOOST_AUTO_TEST_CASE(TestDayofmonth) {
  ...
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn DAYOFMONTH(dateField)} FROM TestType", 29);
  CheckSingleResult< SQLINTEGER >(
...
BOOST_AUTO_TEST_CASE(TestDayofweek) {
  ...
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn DAYOFWEEK(dateField)} FROM TestType", 2);
  CheckSingleResult< SQLINTEGER >(
...
BOOST_AUTO_TEST_CASE(TestDayofyear) {
  ...
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn DAYOFYEAR(dateField)} FROM TestType", 242);
  CheckSingleResult< SQLINTEGER >(
...
BOOST_AUTO_TEST_CASE(TestExtract) {
  ...
  in.timestampField =
      common::MakeTimestampGmt(2016, 2, 24, 13, 45, 23, 580695103);

  testCache.Put(1, in);

  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn EXTRACT(YEAR FROM timestampField)} FROM TestType", 2016);
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn EXTRACT(MONTH FROM timestampField)} FROM TestType", 2);
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn EXTRACT(DAY FROM timestampField)} FROM TestType", 24);
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn EXTRACT(HOUR FROM timestampField)} FROM TestType", 13);
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn EXTRACT(MINUTE FROM timestampField)} FROM TestType", 45);
  CheckSingleResult< SQLINTEGER >(
...
BOOST_AUTO_TEST_CASE(TestYear) {
  ...
  CheckSingleResult< SQLINTEGER >(
      "SELECT {fn YEAR(timestampField)} FROM TestType", 2016);
}
