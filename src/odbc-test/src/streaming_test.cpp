...
struct StreamingTestSuiteFixture : odbc::OdbcTestSuite {
  ...
  void InsertTestStrings2(SQLHSTMT stmt0, int32_t begin, int32_t end) {
    SQLCHAR req[] =
        "INSERT INTO TestType(_key, i32Field, strField) VALUES(?, 42, ?)";

    InsertTestStrings(req, stmt0, begin, end);
  }

  void InsertTestStrings(SQLCHAR* req, SQLHSTMT stmt0, int32_t begin,
                         int32_t end) {
    ...
    // Binding parameters.
    ret = SQLBindParameter(stmt0, 1, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_BIGINT,
                           0, 0, &key, 0, 0);

    if (!SQL_SUCCEEDED(ret))
      BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt0));

    ret = SQLBindParameter(stmt0, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR,
  ...
  void CheckValues(int32_t begin, int32_t end) {
    SQLCHAR req[] =
        "SELECT _key, strField FROM TestType WHERE _key >= ? AND _key < ? "
        "ORDER BY _key";
    ...
    for (int32_t i = begin; i < end; ++i) {
      ...
      BOOST_CHECK_EQUAL(
          GetTestString(i),
          std::string(strField, static_cast< size_t >(strFieldLen)));
    }
...
BOOST_AUTO_TEST_CASE(TestStreamingSimple) {
  ...
  SQLRETURN res =
      ExecQuery("set streaming on batch_size 100 flush_frequency 100");
...
BOOST_AUTO_TEST_CASE(TestStreamingNotAllowedOverwrite) {
  ...
  SQLRETURN res =
      ExecQuery("set streaming 1 allow_overwrite off batch_size 10");
