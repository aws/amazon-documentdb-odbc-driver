...
struct SslQueriesTestSuiteFixture : odbc::OdbcTestSuite {
   ...
   * Constructor.
   */
  SslQueriesTestSuiteFixture() : OdbcTestSuite() {
  ...
  std::string MakeDefaultConnectionString() {
    ...
    connectString << "DRIVER={Apache Ignite};"
                     ...
                     "SSL_MODE=require;"
                     "SSL_KEY_FILE="
                  << cfgDirPath << Fs << "ssl" << Fs
                  << "client_full.pem;"
                     "SSL_CERT_FILE="
                  << cfgDirPath << Fs << "ssl" << Fs
                  << "client_full.pem;"
                     "SSL_CA_FILE="
                  << cfgDirPath << Fs << "ssl" << Fs << "ca.pem;";
...
BOOST_AUTO_TEST_CASE(TestConnectionSslReject) {
  ...
  codes.insert(std::string(
      "08S01"));  // Communication link failure. Returned on newest UnixODBC.
