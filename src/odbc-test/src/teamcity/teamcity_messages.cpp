...
class RaiiMessage {
 public:
  RaiiMessage(const char* const name, std::ostream& out) : m_out(out) {
  ...
  void writePropertyIfNonEmpty(const char* const name,
...
void TeamcityMessages::testFinished(const std::string& name,
  ...
  if (durationMs >= 0) {
    /// \bug W/ some locales it is possible to get a number w/ a number
    /// separator(s)! \todo Make a test for that!
