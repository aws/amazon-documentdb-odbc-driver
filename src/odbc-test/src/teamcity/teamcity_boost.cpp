...
// In 1.59.0, they changed the name of this enum value, so we have to this hacky
// thing...
...
#define UNIT_TEST_CASE boost::unit_test::tut_case
#define CURRENT_TEST_NAME \
  boost::unit_test_framework::framework::current_test_case().p_name
#endif
...
class TeamcityBoostLogFormatter
 ...
 public:
  ...
  virtual void test_unit_start(std::ostream&,
                               const boost::unit_test::test_unit&);
  virtual void test_unit_finish(std::ostream&,
                                const boost::unit_test::test_unit&,
                                unsigned long);

  virtual void log_entry_start(std::ostream&,
  ...
  virtual void log_entry_context(std::ostream& os, boost::unit_test::log_level,
...
void TeamcityBoostLogFormatter::log_start(
    std::ostream& /*out*/, boost::unit_test::counter_t /*test_cases_amount*/) {
}
...
#if BOOST_VERSION >= 107000
// Since v1.70.0 the second argument indicates whether build info should be
// logged or not See boostorg/test.git:7e20f966dca4e4b49585bbe7654334f31b35b3db
void TeamcityBoostLogFormatter::log_build_info(std::ostream& os,
                                               bool log_build_info) {
  if (log_build_info)
    this->log_build_info(os);
}
#endif

void TeamcityBoostLogFormatter::test_unit_start(
    std::ostream& out, const boost::unit_test::test_unit& tu) {
...
void TeamcityBoostLogFormatter::test_unit_finish(
  ...
  const boost::unit_test::test_results& tr =
...
void TeamcityBoostLogFormatter::log_entry_start(
    std::ostream& out, const boost::unit_test::log_entry_data& entry_data,
    log_entry_types /*let*/) {
  std::stringstream ss(std::ios_base::out);
...
void TeamcityBoostLogFormatter::log_entry_value(
    std::ostream& out, boost::unit_test::const_string value) {
  out << value;
...
void TeamcityBoostLogFormatter::log_exception(
    std::ostream& out, const boost::unit_test::log_checkpoint_data&,
...
void TeamcityBoostLogFormatter::log_exception_start(
    std::ostream& out, const boost::unit_test::log_checkpoint_data& data,
    const boost::execution_exception& excpt) {
  log_exception(out, data, excpt.what());
}

void TeamcityBoostLogFormatter::test_unit_skipped(
    std::ostream&, const boost::unit_test::test_unit& tu,
...
void TeamcityBoostLogFormatter::entry_context_start(
    std::ostream& out, boost::unit_test::log_level l) {
  const std::string& initial_msg =
      (l == boost::unit_test::log_successful_tests ? ASSERT_CTX : FAILURE_CTX);
  out << initial_msg;
  currentContextDetails = initial_msg;
}

void TeamcityBoostLogFormatter::log_entry_context(
    std::ostream& out, boost::unit_test::const_string ctx) {
  out << "\n " << ctx;
...
void TeamcityBoostLogFormatter::entry_context_finish(std::ostream& out) {
  out.flush();
  messages.testOutput(CURRENT_TEST_NAME, currentContextDetails, flowId,
