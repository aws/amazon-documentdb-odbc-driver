...
std::string GetStrColumnValue(size_t rowIdx) {
  ...
  generator << "Column 2 test string, row num: " << rowIdx
            << ". Some trailing bytes";
...
void CheckRowData(Row& row, size_t rowIdx) {
  ...
  ApplicationDataBuffer appLongBuf(type_traits::OdbcNativeType::AI_SIGNED_LONG,
                                   &longBuf, sizeof(longBuf), &reslen);
  ApplicationDataBuffer appStrBuf(type_traits::OdbcNativeType::AI_CHAR, &strBuf,
                                  sizeof(strBuf), &reslen);
  ApplicationDataBuffer appGuidBuf(type_traits::OdbcNativeType::AI_GUID,
                                   &guidBuf, sizeof(guidBuf), &reslen);
  ApplicationDataBuffer appBitBuf(type_traits::OdbcNativeType::AI_BIT, &bitBuf,
                                  sizeof(bitBuf), &reslen);

  // Checking size.
  BOOST_REQUIRE(row.GetSize() == 4);

  // Checking 1st column.
  BOOST_REQUIRE(row.ReadColumnToBuffer(1, appLongBuf)
                == ConversionResult::AI_SUCCESS);
  BOOST_REQUIRE_EQUAL(static_cast< size_t >(longBuf), rowIdx * 10);

  // Checking 2nd column.
  BOOST_REQUIRE(row.ReadColumnToBuffer(2, appStrBuf)
                == ConversionResult::AI_SUCCESS);
  ...
  // Checking 3rd column.
  BOOST_REQUIRE(row.ReadColumnToBuffer(3, appGuidBuf)
                == ConversionResult::AI_SUCCESS);
  ...
  // Checking 4th column.
  BOOST_REQUIRE(row.ReadColumnToBuffer(4, appBitBuf)
                == ConversionResult::AI_SUCCESS);
