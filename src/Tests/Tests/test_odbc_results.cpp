...
// const test_string multi_type_data_set =
// CREATE_STRING("kibana_sample_data_types");
...
const test_string multi_col =
    CREATE_STRING("null, 1, VARCHAR '2', DOUBLE '3.3', true");
...
inline void BindColumns(std::vector< std::vector< char > >& cols,
    ...
    for (size_t i = 0; i < cols.size(); i++) {
        ret = SQLBindCol(*hstmt, (SQLUSMALLINT)i + 1, SQL_C_CHAR, &cols[i][0],
                         buffer_size, &lens[i][0]);
        LogAnyDiagnostics(SQL_HANDLE_STMT, *hstmt, ret);
        ASSERT_TRUE(SQL_SUCCEEDED(ret));
    }
}

void ExecuteQuery(const test_string& column, const test_string& table,
...
template < class T >
auto CompareSQLType =
    [](const T& expected, const T& actual) { EXPECT_EQ(expected, actual); };
...
auto CompareIntervalStruct = [](const SQL_INTERVAL_STRUCT& expected,
    ...
    if (expected.interval_type == SQL_IS_YEAR_TO_MONTH) {
        ...
        EXPECT_EQ(expected.intval.day_second.hour,
                  actual.intval.day_second.hour);
        EXPECT_EQ(expected.intval.day_second.minute,
                  actual.intval.day_second.minute);
        EXPECT_EQ(expected.intval.day_second.second,
                  actual.intval.day_second.second);
        EXPECT_EQ(expected.intval.day_second.fraction,
                  actual.intval.day_second.fraction);
    } else {
        FAIL();
    }
};
...
auto ConstructIntervalStruct =
    [](SQLINTERVAL type, SQLSMALLINT sign, SQLUINTEGER year, SQLUINTEGER month,
...
TEST_F(TestSQLBindCol, InvalidColIndex0) {
    ...
    SQLRETURN ret = SQLBindCol(m_hstmt, (SQLUSMALLINT)1, SQL_C_CHAR,
                               &cols[0][0], buffer_size, &lens[0][0]);
    LogAnyDiagnostics(SQL_HANDLE_STMT, m_hstmt, ret);
    ASSERT_TRUE(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(m_hstmt, (SQLUSMALLINT)0, SQL_C_CHAR, &cols[0][0],
                     buffer_size, &lens[0][0]);
    EXPECT_EQ(SQL_ERROR, ret);
...
TEST_F(TestSQLBindCol, InvalidColIndex2) {
    ...
    LogAnyDiagnostics(SQL_HANDLE_STMT, m_hstmt, ret);
    ASSERT_TRUE(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(m_hstmt, (SQLUSMALLINT)2, SQL_C_CHAR, &cols[1][0],
                     buffer_size, &lens[1][0]);
    EXPECT_EQ(SQL_ERROR, ret);
    EXPECT_TRUE(CheckSQLSTATE(SQL_HANDLE_STMT, m_hstmt,
...
TEST_F(TestSQLBindCol, InsufficientSpace) {
    ...
    EXPECT_EQ(tchar_to_string(msg_buffer.data()),
              tchar_to_string(
                  (SQLTCHAR*)CREATE_STRING("Fetched item was truncated.")));
    EXPECT_EQ(tchar_to_string(data_buffer.data()),
              tchar_to_string((SQLTCHAR*)colret.c_str()));
}
...
TEST_F(TestSQLGetData, TIMESERIES_TO_SQL_C_CHAR_ARRAY) {
    SQLRETURN ret = SQL_ERROR;
    test_string statement = CREATE_STRING(
        ...
        CREATE_STRING("14:18:30.123456789' AS binned_timestamp, ARRAY[1,2,3] ")
            CREATE_STRING("AS data FROM ODBCTest.IoT LIMIT ")
                CREATE_STRING("1), interpolated_timeseries AS(SELECT ")
                    CREATE_STRING(
                        "CREATE_TIME_SERIES(binned_timestamp, data) FROM "
                        "binned_timeseries) ")
    ...
    SQLLEN indicator = 0;
    ret = SQLGetData(m_hstmt, 1, SQL_C_CHAR, data, 1024, &indicator);
    test_string expected = CREATE_STRING(
        "[{time: 2021-03-05 14:18:30.123456789, value: [1, 2, 3]}]");
...
TEST_F(TestSQLGetData, TIMESERIES_TO_SQL_C_CHAR_ROW) {
    ...
    test_string statement = CREATE_STRING(
        "WITH binned_timeseries AS(SELECT TIMESTAMP'2021-03-05 ")
        CREATE_STRING(
            "14:18:30.123456789' AS binned_timestamp, CAST(ROW(9.9, 19) AS ")
            CREATE_STRING(
                "ROW(sum DOUBLE, count INTEGER)) AS data FROM ODBCTest.IoT "
                "LIMIT ")
                CREATE_STRING("1), interpolated_timeseries AS(SELECT ")
                    CREATE_STRING(
                        "CREATE_TIME_SERIES(binned_timestamp, data) FROM "
    ...
    ret = SQLGetData(m_hstmt, 1, SQL_C_CHAR, data, 1024, &indicator);
    test_string expected = CREATE_STRING(
        "[{time: 2021-03-05 14:18:30.123456789, value: (9.9, 19)}]");
    CompareStrNumBytes(expected, indicator, data, ret);
    LogAnyDiagnostics(SQL_HANDLE_STMT, m_hstmt, ret);
}
...
TEST_F(TestSQLGetData, TIMESERIES_TO_SQL_C_CHAR_ARRAY_ROW_COMBINATION) {
    ...
    test_string statement =
        CREATE_STRING("WITH binned_timeseries AS(SELECT TIMESTAMP'2021-03-05 ")
            CREATE_STRING(
                ...
                CREATE_STRING("AS data FROM ODBCTest.IoT LIMIT ") CREATE_STRING(
    ...
    ret = SQLGetData(m_hstmt, 1, SQL_C_CHAR, data, 1024, &indicator);
    test_string expected = CREATE_STRING(
        "[{time: 2021-03-05 14:18:30.123456789, value: (null, [[(12345, [1, 2, "
        "3])]])}]");
    CompareStrNumBytes(expected, indicator, data, ret);
    LogAnyDiagnostics(SQL_HANDLE_STMT, m_hstmt, ret);
}
...
TEST_F(TestSQLGetData, ARRAY_TO_SQL_C_CHAR) {
    test_string columns = CREATE_STRING(
        "ARRAY[ARRAY[ARRAY[ARRAY[1.1, 2.3], ARRAY[1.1, 2.3]]], "
        "ARRAY[ARRAY[ARRAY[1.1, 2.3], ARRAY[1.1, 2.3]]]], "
...
TEST_F(TestSQLGetData, ROW_TO_SQL_C_WCHAR) {
    ...
    test_string expected =
        CREATE_STRING("(((3, 10, true), [[1.0, 2.0], [1.1, 2.2]]))");
    CompareStrNumBytes(expected, indicator, data, ret);
...
TEST_F(TestSQLGetData, ARRAY_ROW_NULL_TO_SQL_C_CHAR) {
    test_string columns = CREATE_STRING(
        "Array[Row(null), Row(NULL)], Row(Array[null], Array[NULL])");
    QueryFetch(columns, table_name, single_row, &m_hstmt);
    SQLCHAR data[1024] = {0};
    SQLCHAR data2[1024] = {0};
    SQLLEN indicator = 0;
...
TEST_F(TestSQLGetData, DATE_TO_SQL_C_CHAR) {
    test_string columns =
        CREATE_STRING("DATE \'2021-01-02\', DATE \'2021-11-20\'");
    QueryFetch(columns, table_name, single_row, &m_hstmt);
    SQLCHAR data[1024] = {0};
    SQLLEN indicator = 0;
    SQLRETURN ret = SQL_ERROR;
...
TEST_F(TestSQLGetData, DATE_TO_SQL_C_TYPE_TIME) {
    ...
    std::vector< std::pair< TIME_STRUCT, SQLTCHAR* > > expected;
    expected.push_back(std::make_pair(TIME_STRUCT{0, 0, 0},
                                      SQLSTATE_RESTRICTED_DATA_TYPE_ERROR));
    expected.push_back(std::make_pair(TIME_STRUCT{0, 0, 0},
                                      SQLSTATE_RESTRICTED_DATA_TYPE_ERROR));
...
TEST_F(TestSQLGetData, TIME_TO_SQL_C_CHAR) {
    test_string columns = CREATE_STRING("TIME \'18:01:13.000000000\',")
...
TEST_F(TestSQLGetData, TIME_TO_SQL_C_TYPE_DATE) {
    test_string columns = CREATE_STRING(
        "TIME \'18:01:13.524000000\', TIME \'06:39:45.123456789\'");

    std::vector< std::pair< DATE_STRUCT, SQLTCHAR* > > expected;
    expected.push_back(std::make_pair(DATE_STRUCT{0, 0, 0},
                                      SQLSTATE_RESTRICTED_DATA_TYPE_ERROR));
    expected.push_back(std::make_pair(DATE_STRUCT{0, 0, 0},
                                      SQLSTATE_RESTRICTED_DATA_TYPE_ERROR));
...
TEST_F(TestSQLGetData, VARCHAR_TO_SQL_C_BIT) {
    ...
    test_string columns =
        CREATE_STRING("VARCHAR\'0.0") CREATE_STRING("\', VARCHAR\'")
        + convert_to_test_string(v1) + CREATE_STRING("\', VARCHAR\'")
        + convert_to_test_string(v2) + CREATE_STRING("\', VARCHAR\'")
        + convert_to_test_string(v3) + CREATE_STRING("\', VARCHAR\'   ")
...
TEST_F(TestSQLGetData, VARCHAR_TO_SQL_C_SLONG) {
    ...
    unsigned long long v9 = (unsigned long long)INT32_MAX + 1;  // overflow
    test_string columns =
        CREATE_STRING("VARCHAR\'") + convert_to_test_string(v1)
        + CREATE_STRING("\', VARCHAR\'") + convert_to_test_string(v2)
        + CREATE_STRING("\', VARCHAR\'") + convert_to_test_string(v3)
...
TEST_F(TestSQLGetData, VARCHAR_TO_SQL_C_LONG) {
    ...
    unsigned long long v8 = (unsigned long long)INT32_MAX + 1;  // overflow
    test_string columns =
        CREATE_STRING("VARCHAR\'") + convert_to_test_string(v1)
        + CREATE_STRING("\', VARCHAR\'") + convert_to_test_string(v2)
        + CREATE_STRING("\', VARCHAR\'") + convert_to_test_string(v3)
...
TEST_F(TestSQLGetData, VARCHAR_TO_SQL_C_CHAR) {
    int v1 = -293719;
    int v2 = 741370;
    test_string columns = CREATE_STRING("VARCHAR\'")
                          + convert_to_test_string(v1)
                          + CREATE_STRING("\', VARCHAR\'")
                          + convert_to_test_string(v2) + CREATE_STRING("\'");
    QueryFetch(columns, table_name, single_row, &m_hstmt);
...
TEST_F(TestSQLGetData, VARCHAR_TO_SQL_C_TYPE_TIMESTAMP) {
    test_string columns = CREATE_STRING(
        "VARCHAR \'2021-01-02 18:01:13.000000000\',")
        CREATE_STRING("VARCHAR \'2021-11-20 06:39:45.123456789\',")
            CREATE_STRING("VARCHAR \'2021-11-20 06:39:45.12345\',")
                CREATE_STRING("VARCHAR \'2021-11-20 06:39:45\',") CREATE_STRING(
                    "VARCHAR \'2021-11-20 06:39\',")
    ...
    test_string columns = CREATE_STRING(
        ...
        CREATE_STRING("VARCHAR \'2021-11-20 06:39:45.123456789\',")
            CREATE_STRING("VARCHAR \'2021-11-20 06:39:45.12345\',")
                CREATE_STRING("VARCHAR \'2021-11-20 06:39:45\',") CREATE_STRING(
                    "VARCHAR \'2021-11-20 06:39\',")
                    CREATE_STRING("VARCHAR \'2021-11-20 06\',") CREATE_STRING(
                        "VARCHAR \'   2021-01-02 18:01:13.000000000   \',")
                        CREATE_STRING("VARCHAR \'2021-11-20\',")
                            CREATE_STRING("VARCHAR \'2021-11\',")
        // invalid
...
TEST_F(TestSQLGetData, VARCHAR_TO_INTERVAL_YEAR_TO_MONTH) {
    test_string columns = CREATE_STRING("VARCHAR \'1-0',")
        CREATE_STRING("VARCHAR \'0-1\',") CREATE_STRING("VARCHAR \'-1-0\',")
            CREATE_STRING("VARCHAR \'-0-1\',") CREATE_STRING("VARCHAR \'0-0\',")
...
TEST_F(TestSQLGetData, VARCHAR_TO_INTERVAL_DAY_TO_SECOND) {
    test_string columns = CREATE_STRING("VARCHAR \'1 00:00:00.000000000\',")
        CREATE_STRING("VARCHAR \'0 01:00:00.000000000\',")
            CREATE_STRING("VARCHAR \'0 00:01:00.000000000\',") CREATE_STRING(
                "VARCHAR \'0 00:00:01.000000000\',")
...
TEST_F(TestSQLGetData, INTERVAL_YEAR_TO_MONTH_TO_SQL_C_CHAR) {
    test_string columns = CREATE_STRING("1year,") CREATE_STRING("1month,")
        CREATE_STRING("-1year,") CREATE_STRING("-1month,")
            CREATE_STRING("0year,") CREATE_STRING("0month");
    QueryFetch(columns, table_name, single_row, &m_hstmt);
    SQLCHAR data[1024] = {0};
    SQLCHAR data2[1024] = {0};
    SQLCHAR data3[1024] = {0};
    ...
    test_string columns = CREATE_STRING(
        ...
        CREATE_STRING(
            ...
            CREATE_STRING(
                "Array[Row(null), Row(NULL)], Row(Array[null], Array[NULL]), ")
                CREATE_STRING(
                    "Array[Row(null), Row(NULL)], Row(Array[null], "
                    "Array[NULL]), ")
                    CREATE_STRING(
                        "Array[Row(null), Row(NULL)], Row(Array[null], "
                        "Array[NULL]), ")
...
TEST_F(TestSQLDescribeCol, ROW_COLUMN) {
    SQLRETURN ret = SQL_ERROR;
    test_string columns = CREATE_STRING(
        "ROW(ROW(ROW(INTEGER '03', BIGINT '10', true), "
        "ARRAY[ARRAY[1,2],ARRAY[1.1,2.2]]))");
    QueryFetch(columns, table_name, single_row, &m_hstmt);
    SQLTCHAR column_name[60];
    SQLSMALLINT column_name_length;
    SQLSMALLINT data_type;
...
TEST_F(TestSQLDescribeCol, MULTIPLE_COLUMNS) {
    SQLRETURN ret = SQL_ERROR;
    test_string columns = CREATE_STRING(
        "INTEGER\'1\', DOUBLE \'1.0\', BIGINT \'2147483648\', true");
    QueryFetch(columns, table_name, single_row, &m_hstmt);
    SQLTCHAR column_name[60];
    SQLSMALLINT column_name_length;
    SQLSMALLINT data_type;
