    ...
    void SetUp() override {
        ...
        if (m_hstmt != SQL_NULL_HSTMT) {
            ASSERT_EQ(SQLSetCursorName(
                          m_hstmt, AS_SQLTCHAR(m_cursor_name.c_str()), SQL_NTS),
                      SQL_SUCCESS);
        }
    }
...
TEST_F(TestSQLPrepare, ERROR_WITH_PARAM) {
    test_string query =
        CREATE_STRING("SELECT 1 FROM ODBCTest.IoT where time = ? LIMIT 1");
    SQLRETURN ret = SQLPrepare(m_hstmt, (SQLTCHAR*)query.c_str(), SQL_NTS);
    EXPECT_EQ(SQL_ERROR, ret);
    LogAnyDiagnostics(SQL_HANDLE_STMT, m_hstmt, ret);
}
...
TEST_F(TestSQLPrepare, PREPARE_METADATA) {
    test_string query =
        CREATE_STRING("SELECT 1, 2, 3, 4 FROM ODBCTest.IoT LIMIT 1");
    SQLRETURN ret = SQLPrepare(m_hstmt, (SQLTCHAR*)query.c_str(), SQL_NTS);
    EXPECT_EQ(SQL_SUCCESS, ret);
    LogAnyDiagnostics(SQL_HANDLE_STMT, m_hstmt, ret);
...
TEST_F(TestSQLFetch, INVALID_CURSOR_STATE) {
    ...
    EXPECT_TRUE(
        CheckSQLSTATE(SQL_HANDLE_STMT, m_hstmt, SQLSTATE_INVALID_CURSUR_STATE));
}
...
TEST_F(TestSQLFetch, FRACTIONAL_TRUNCATION) {
    test_string query =
        CREATE_STRING("SELECT VARCHAR \'1.5\' FROM ODBCTest.IoT LIMIT 1");
    SQLRETURN ret = SQLExecDirect(m_hstmt, (SQLTCHAR*)query.c_str(), SQL_NTS);
    EXPECT_TRUE(SQL_SUCCEEDED(ret));
    SQLCHAR data;
    SQLLEN indicator = 0;
...
TEST_F(TestSQLFetch, RIGHT_TRUNCATION) {
    test_string query =
        CREATE_STRING("SELECT VARCHAR \'741370\' FROM ODBCTest.IoT LIMIT 1");
    SQLRETURN ret = SQLExecDirect(m_hstmt, (SQLTCHAR*)query.c_str(), SQL_NTS);
    EXPECT_TRUE(SQL_SUCCEEDED(ret));
    SQLCHAR data[1024] = {0};
    SQLLEN indicator = 0;
...
TEST_F(TestSQLFetch, OUT_OF_RANGE) {
    test_string query =
        CREATE_STRING("SELECT VARCHAR \'2\' FROM ODBCTest.IoT LIMIT 1");
    SQLRETURN ret = SQLExecDirect(m_hstmt, (SQLTCHAR*)query.c_str(), SQL_NTS);
    EXPECT_TRUE(SQL_SUCCEEDED(ret));
    SQLCHAR data;
    SQLLEN indicator = 0;
...
TEST_F(TestSQLFetch, STRING_CONVERSION_ERROR) {
    test_string query =
        CREATE_STRING("SELECT VARCHAR \'1.a\' FROM ODBCTest.IoT LIMIT 1");
    SQLRETURN ret = SQLExecDirect(m_hstmt, (SQLTCHAR*)query.c_str(), SQL_NTS);
    EXPECT_TRUE(SQL_SUCCEEDED(ret));
    SQLCHAR data;
    SQLLEN indicator = 0;
...
TEST_F(TestSQLExecDirect, Success_100) {
    int limit = 100;
    test_string query = CREATE_STRING("SELECT * FROM ODBCTest.IoT LIMIT ")
                        + to_test_string(std::to_string(limit));
    SQLRETURN ret = SQLExecDirect(m_hstmt, (SQLTCHAR*)query.c_str(), SQL_NTS);
...
TEST_F(TestSQLCancel, QueryInProgressMultithread) {
    test_string query = CREATE_STRING("SELECT * FROM ODBCTest.IoT");
    std::thread th1([&]() {
