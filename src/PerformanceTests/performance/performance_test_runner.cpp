...
bool performance::PerformanceTestRunner::checkFileExtension(
...
bool performance::PerformanceTestRunner::checkDSN(const std::string dsn) {
  SQLHENV env;
  SQLRETURN ret;
  ...
  _error_msg = "DSN ERROR: " + dsn + " is not installed.";
  return false;
}

bool performance::PerformanceTestRunner::checkCSVHeaders() {
  ...
  for (std::size_t column = 0; column < _cell_refs.size(); ++column) {
    ...
    if (cell.getType() == Csv::CellType::String) {
      ...
      _error_msg =
          "INPUT FILE ERROR: header values must be of string type in input "
          "file " +
          _input_file + ".";
      return false;
    }
  ...
  if (_headers.idx_query < 0 || _headers.idx_limit < 0 ||
    ...
    _error_msg = "INPUT FILE ERROR: header value(s) missing in input file " +
                 _input_file + ".";
    return false;
  }
  return true;
...
std::string performance::PerformanceTestRunner::getTestName(const Csv::CellReference& cell_test_name) const  {
...
int performance::PerformanceTestRunner::getIterationCount(const Csv::CellReference& cell_iteration_count) const  {
...
bool performance::PerformanceTestRunner::hasCommaOrNewLine(const std::string str) const  {
...
void performance::PerformanceTestRunner::recordExecBindFetch(
  ...
  for (size_t iter = 0; iter < test_case.num_iterations; iter++) {
    ...
    while (SQLExtendedFetch(*hstmt, SQL_FETCH_NEXT, 0, &rows_fetched, row_status) == SQL_SUCCESS) {
      row_count++;
    }
    auto time_bind_fetch_end = std::chrono::steady_clock::now();

    // Store bind and fetch time
    time_bind_fetch_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
...
void performance::PerformanceTestRunner::calcStats(TestCase& test_case) {
  ...
  if (size < 1) {
    throw std::runtime_error(
        ...
        "Potential errors may be incorrect iteration count or SQLExecDirect "
  ...
  try {
    if (size == 1) {
      ...
      test_case.stat_info_exec.max =
          *std::max_element(test_case.time_exec_ms.begin(), test_case.time_exec_ms.end());
      test_case.stat_info_exec.min =
          *std::min_element(test_case.time_exec_ms.begin(), test_case.time_exec_ms.end());
      test_case.stat_info_bind_fetch.max = *std::max_element(
          test_case.time_bind_fetch_ms.begin(), test_case.time_bind_fetch_ms.end());
      test_case.stat_info_bind_fetch.min = *std::min_element(
          test_case.time_bind_fetch_ms.begin(), test_case.time_bind_fetch_ms.end());
      ...
      std::sort(test_case.time_bind_fetch_ms.begin(), test_case.time_bind_fetch_ms.end());

      test_case.stat_info.median = (size % 2)
                                       ? test_case.time_ms[size / 2]
                                       : ((test_case.time_ms[(size / 2) - 1] +
                                           test_case.time_ms[size / 2]) /
                                          2);
      test_case.stat_info_exec.median = (size % 2)
...
void performance::PerformanceTestRunner::reportTime(const TestCase& test_case) {
  ...
  std::cout << sync_query;
  std::cout << test_case.query << std::endl;
  ...
  if (_output_mode == 0 || _output_mode == 3) {
    ...
    for (size_t i = 0; i < test_case.time_ms.size(); i++) {
      ...
      if (i != (test_case.time_ms.size() - 1)) std::cout << ", ";
    }
    std::cout << "\n\n";
    ...
    std::cout << sync_mean << test_case.stat_info_exec.avg << " ms" << std::endl;
    ...
    for (size_t i = 0; i < test_case.time_exec_ms.size(); i++) {
      ...
      if (i != (test_case.time_exec_ms.size() - 1)) std::cout << ", ";
    }
    std::cout << "\n\n";
    ...
    std::cout << sync_mean << test_case.stat_info_bind_fetch.avg << " ms"
              << std::endl;
    // std::cout << sync_stdev << test_case.stat_info_bind_fetch.stdev << " ms"
    // << std::endl;
...
performance::PerformanceTestRunner::PerformanceTestRunner(
  ...
  if (!checkFileExtension(test_plan_csv, ".csv")) {
    throw std::runtime_error(_error_msg);
  }

  if (!checkFileExtension(output_file_csv, ".csv")) {
    throw std::runtime_error(_error_msg);
  }
...
bool performance::PerformanceTestRunner::readPerformanceTestPlan() {
  ...
  if (!checkCSVHeaders()) {
    // error msg set in checkCSVHeaders
    return false;
  }
  // _headers data member is set

  return true;
}

bool performance::PerformanceTestRunner::setupConnection() {
  SQLTCHAR out_conn_string[1024];
  SQLSMALLINT out_conn_string_length;
  SQLRETURN ret;
  ...
  if (ret == SQL_INVALID_HANDLE || ret == SQL_ERROR) {
    _error_msg = "SQLAllocHandle ERROR: failed to allocate environment handle.";
    return false;
  }

  ret = SQLSetEnvAttr(_env, SQL_ATTR_ODBC_VERSION, (void*)SQL_OV_ODBC3, 0);
  if (ret == SQL_INVALID_HANDLE || ret == SQL_ERROR) {
    _error_msg = "SQLSetEnvAttr ERROR: failed to set environment attribute.";
    return false;
  }

  ret = SQLAllocHandle(SQL_HANDLE_DBC, _env, &_conn);
  if (ret == SQL_INVALID_HANDLE || ret == SQL_ERROR) {
    _error_msg = "SQLAllocHandle ERROR: failed to allocate connection handle.";
    return false;
  }
  ...
  if (ret == SQL_INVALID_HANDLE || ret == SQL_ERROR) {
    ...
    _error_msg = "SQLDriverConnect ERROR: no data in DSN = " + _dsn + ".";
    return false;
  } else if (ret == SQL_STILL_EXECUTING) {
    _error_msg =
        "SQLDriverConnect ERROR: still trying to connect to DSN = " + _dsn +
        ".";
    return false;
  }
  ...
  if (ret == SQL_INVALID_HANDLE || ret == SQL_ERROR) {
    _error_msg =
        "SQLAllocHandle ERROR: failed to allocate connection statement handle.";
    return false;
  }
...
bool performance::PerformanceTestRunner::runPerformanceTestPlan() {
  ...
  for (std::size_t row = 1; row < num_test_cases; ++row) {
    try {
      ...
      _error_msg = "TESTCASE ERROR: " + (std::string)err.what() +
                   std::to_string(row) + ".";
      ofs.close();
      return false;
    }
