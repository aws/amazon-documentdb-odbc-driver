...
struct Person {
  ...
  std::string ToString() const {
    ...
    oss << "Person [orgId=" << orgId << ", lastName=" << lastName
...
// Person key with affinity Organization info. If we want to to collocate
// Persons with the same value of orgId we need to put struct as a cache key
// that contains fields: 1) id of the record (person id or serial id number) 2)
// collocation column info (orgId). This is required because of constraint :
// affinity key must be part of the key.
struct PersonKey {
  ...
  std::string ToString() const {
    ...
    oss << "PersonKey [id=" << id << ", orgIdAff=" << orgIdAff << "]";
    return oss.str();
  }
...
struct BinaryType< examples::Person >
    : BinaryTypeDefaultAll< examples::Person > {
...
struct BinaryType< examples::PersonKey >
    : BinaryTypeDefaultAll< examples::PersonKey > {
  static void GetTypeName(std::string& dst) {
    dst = "PersonKey";
  }
