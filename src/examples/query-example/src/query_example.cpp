...
void DoSqlQueryWithDistributedJoin() {
  ...
  std::cout
      << "Following people are 'ApacheIgnite' employees (distributed join): "
      << std::endl;
  ...
  std::cout << "Following people are 'Other' employees (distributed join): "
            << std::endl;
...
/**
 * Example for SQL-based fields queries that return only person name and company
 * it works in.
 *
 * Note that SQL Fields Query can only be performed using fields that have been
 * listed in "QueryEntity" been of the config.
...
void DoSqlQueryWithJoin() {
  ...
  std::cout << "Names of all employees and organizations they belong to: "
            << std::endl;
...
void DoSqlQueryWithFunction() {
  ...
  // Execute query to get names of all employees.
  QueryFieldsCursor cursor = cache.Query(
      SqlFieldsQuery("select concat(firstName, ' ', lastName) from Person"));

  // Print names.
  std::cout << "Names of all employees: " << std::endl;

  // In this particular case each row will have one element with full name of an
  // employees.
  while (cursor.HasNext()) {
...
void DoTextQuery() {
  ...
  for (ResVector::const_iterator i = masters.begin(); i != masters.end(); ++i)
    std::cout << i->GetKey().ToString() << " : " << i->GetValue().ToString()
              << std::endl;

  std::cout << std::endl;

  std::cout << "Following people have 'Bachelor' in their resumes: "
            << std::endl;

  // Printing second result set.
  for (ResVector::const_iterator i = bachelors.begin(); i != bachelors.end();
       ++i)
    std::cout << i->GetKey().ToString() << " : " << i->GetValue().ToString()
              << std::endl;
...
void DoScanQuery() {
  ...
  for (ResVector::const_iterator i = res.begin(); i != res.end(); ++i) {
    ...
    if (person.salary <= 1000)
      std::cout << i->GetKey().ToString() << " : " << person.ToString()
                << std::endl;
  }
...
int main() {
  try {
    ...
    cfg.springCfgPath =
        "platforms/cpp/examples/query-example/config/query-example.xml";
    ...
    Cache< int64_t, Organization > orgCache =
        ignite.GetCache< int64_t, Organization >(ORG_CACHE);

    // Get person cache instance.
    Cache< PersonKey, Person > personCache =
        ignite.GetCache< PersonKey, Person >(PERSON_CACHE);
    ...
    // Example for SQL-based querying to calculate average salary among all
