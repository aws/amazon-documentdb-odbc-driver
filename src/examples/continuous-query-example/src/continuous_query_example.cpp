 ...
 public:
  ...
  virtual void OnEvent(const CacheEntryEvent< K, V >* evts, uint32_t num) {
    for (uint32_t i = 0; i < num; ++i) {
      std::cout << "Queried entry [key="
                << (evts[i].HasValue() ? evts[i].GetKey() : K())
...
struct RangeFilter : event::CacheEntryEventFilter< int32_t, std::string > {
   ...
   * Default constructor.
   */
  RangeFilter() : rangeBegin(0), rangeEnd(0) {
    // No-op.
  }
   ...
   */
  RangeFilter(const K& from, const K& to) : rangeBegin(from), rangeEnd(to) {
    // No-op.
  }
...
int main() {
  ...
  cfg.springCfgPath =
      "platforms/cpp/examples/continuous-query-example/config/"
      "continuous-query-example.xml";
  ...
  try {
    ...
    binding
        .RegisterCacheEntryEventFilter< RangeFilter< int32_t, std::string > >();
    ...
    continuous::ContinuousQuery< int32_t, std::string > qry(
        MakeReference(listener), MakeReference(filter));

    {
      // Continuous query scope. Query is closed when scope is left.
      continuous::ContinuousQueryHandle< int32_t, std::string > handle =
