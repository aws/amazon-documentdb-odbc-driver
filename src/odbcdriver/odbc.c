...
BOOL WINAPI DllMain(HANDLE hInst, ULONG ul_reason_for_call, LPVOID lpReserved) {
    ...
    switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH:
            ...
            MYLOG(LOG_DEBUG, "exe name=%s", exename);
            break;
        ...
        case DLL_PROCESS_DETACH:
            MYLOG(LOG_DEBUG, "DETACHING %s", DRIVER_FILE_NAME);
...
static void __attribute__((constructor)) databaseodbc_init(void) {
    initialize_global_cs();
}

static void __attribute__((destructor)) databaseodbc_fini(void) {
    finalize_global_cs();
}
