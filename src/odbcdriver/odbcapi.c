...
RETCODE SQL_API SQLDriverConnect(HDBC hdbc, HWND hwnd, SQLCHAR *szConnStrIn,
    ...
    ENTER_CONN_CS(conn);
    CC_clear_error(conn);
    ret = API_DriverConnect(hdbc, hwnd, szConnStrIn, cbConnStrIn, szConnStrOut,
...
RETCODE SQL_API SQLDisconnect(HDBC ConnectionHandle) {
    ...
    MYLOG(LOG_TRACE, "entering for %p", ConnectionHandle);
...
RETCODE SQL_API SQLFreeStmt(HSTMT StatementHandle, SQLUSMALLINT Option) {
    ...
    ret = API_FreeStmt(StatementHandle, Option);
...
RETCODE SQL_API SQLGetCursorName(HSTMT StatementHandle, SQLCHAR *CursorName,
    ...
    ENTER_STMT_CS(stmt);
    SC_clear_error(stmt);
    ret = API_GetCursorName(StatementHandle, CursorName, BufferLength,
                              NameLength);
    LEAVE_STMT_CS(stmt);
    return ret;
}
...
RETCODE SQL_API SQLGetData(HSTMT StatementHandle, SQLUSMALLINT ColumnNumber,
    ...
    ENTER_STMT_CS(stmt);
    SC_clear_error(stmt);
    ret = API_GetData(StatementHandle, ColumnNumber, TargetType, TargetValue,
                        BufferLength, StrLen_or_Ind);
    LEAVE_STMT_CS(stmt);
    return ret;
}
...
RETCODE SQL_API SQLGetFunctions(HDBC ConnectionHandle, SQLUSMALLINT FunctionId,
    ...
    if (FunctionId == SQL_API_ODBC3_ALL_FUNCTIONS)
        ret = API_GetFunctions30(ConnectionHandle, FunctionId, Supported);
    else
        ret = API_GetFunctions(ConnectionHandle, FunctionId, Supported);
...
RETCODE SQL_API SQLGetInfo(HDBC ConnectionHandle, SQLUSMALLINT InfoType,
    ...
    CC_clear_error(conn);
    MYLOG(LOG_TRACE, "entering");
    if ((ret = API_GetInfo(ConnectionHandle, InfoType, InfoValue,
...
RETCODE SQL_API SQLNumResultCols(HSTMT StatementHandle,
    ...
    ENTER_STMT_CS(stmt);
    SC_clear_error(stmt);
    ret = API_NumResultCols(StatementHandle, ColumnCount);
    LEAVE_STMT_CS(stmt);
    return ret;
}
...
RETCODE SQL_API SQLSetCursorName(HSTMT StatementHandle, SQLCHAR *CursorName,
    ...
    ENTER_STMT_CS(stmt);
    SC_clear_error(stmt);
    ret = API_SetCursorName(StatementHandle, CursorName, NameLength);
    LEAVE_STMT_CS(stmt);
    return ret;
}
#endif /* UNICODE_SUPPORTXX */
...
RETCODE SQL_API SQLSpecialColumns(HSTMT StatementHandle,
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_SpecialColumns(StatementHandle, IdentifierType, ctName,
...
RETCODE SQL_API SQLStatistics(HSTMT StatementHandle, SQLCHAR *CatalogName,
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_Statistics(StatementHandle, ctName, NameLength1, scName,
...
RETCODE SQL_API SQLColumnPrivileges(
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_ColumnPrivileges(hstmt, ctName, cbCatalogName, scName,
...
RETCODE SQL_API SQLForeignKeys(
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_ForeignKeys(hstmt, pkctName, cbPkCatalogName, pkscName,
...
RETCODE SQL_API SQLNativeSql(HDBC hdbc, SQLCHAR *szSqlStrIn,
    ...
    ENTER_CONN_CS(conn);
    CC_clear_error(conn);
    ret = API_NativeSql(hdbc, szSqlStrIn, cbSqlStrIn, szSqlStr, cbSqlStrMax,
                          pcbSqlStr);
    LEAVE_CONN_CS(conn);
    return ret;
}
...
RETCODE SQL_API SQLNumParams(HSTMT hstmt, SQLSMALLINT *pcpar) {
    ...
    return ParametersNotSupported((StatementClass *)hstmt, "SQLNumParams");
}
...
RETCODE SQL_API SQLPrimaryKeys(HSTMT hstmt, SQLCHAR *szCatalogName,
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_PrimaryKeys(hstmt, ctName, cbCatalogName, scName,
...
RETCODE SQL_API SQLProcedureColumns(
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_ProcedureColumns(hstmt, ctName, cbCatalogName, scName,
...
RETCODE SQL_API SQLProcedures(HSTMT hstmt, SQLCHAR *szCatalogName,
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_Procedures(hstmt, ctName, cbCatalogName, scName,
...
RETCODE SQL_API SQLTablePrivileges(HSTMT hstmt, SQLCHAR *szCatalogName,
    ...
    if (SC_opencheck(stmt, func))
        ...
        ret = API_TablePrivileges(hstmt, ctName, cbCatalogName, scName,
...
RETCODE SQL_API SQLTransact(HENV henv, HDBC hdbc, SQLUSMALLINT fType) {
    UNUSED(henv, fType);
    ConnectionClass *conn = (ConnectionClass *)hdbc;
    if (conn == NULL)
        return SQL_ERROR;
    ...
    CC_set_error(conn, CONN_NOT_IMPLEMENTED_ERROR,
                 "SQLTransact is not supported.", "SQLTransact");
    return SQL_ERROR;
}
...
RETCODE SQL_API SQLGetConnectOption(HDBC ConnectionHandle, SQLUSMALLINT Option,
    ...
    MYLOG(LOG_TRACE, "entering " FORMAT_USMALLI, Option);
    ENTER_CONN_CS((ConnectionClass *)ConnectionHandle);
    CC_clear_error((ConnectionClass *)ConnectionHandle);
    ret = API_GetConnectOption(ConnectionHandle, Option, Value, NULL, 0);
...
RETCODE SQL_API SQLSetConnectOption(HDBC ConnectionHandle, SQLUSMALLINT Option,
    ...
    ENTER_CONN_CS(conn);
    CC_clear_error(conn);
    ret = API_SetConnectOption(ConnectionHandle, Option, Value);
    LEAVE_CONN_CS(conn);
    return ret;
}
