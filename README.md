...
7. Set the environment variable `DOCUMENTDB_HOME`. On a deverloper's machine, set it to `<repo-folder>\build\odbc\bin\Debug`. The 
...
8. Open a **64-bit** command shell or **64-bit** PowerShell window, **as Administrator**, run the 
   ...
   ``` 
8. More details in `src\DEVNOTES.txt`.
