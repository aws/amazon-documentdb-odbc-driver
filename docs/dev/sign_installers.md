...
```
  signtool sign /sha1 <CertificateHash> '.\Amazon Timestream ODBC Driver-<version>-Windows.msi'
```
...
```
  Set-AuthenticodeSignature '.\Amazon Timestream ODBC Driver-<version>-Windows.msi' -Certificate (Get-ChildItem Cert:\CurrentUser\My -CodeSigningCert) -TimestampServer "http://timestamp.verisign.com/scripts/timstamp.dll"
```
...
```
productsign -sign "Developer ID Installer: Your Apple Account Name (**********)" "~/Desktop/Amazon Timestream ODBC Driver-<version>-Darwin.pkg" "~/Desktop/signed-Amazon Timestream ODBC Driver-<version>-Darwin.pkg"
```

- Test installer package using [spctl](http://www.manpagez.com/man/8/spctl/)
```
  spctl -a -v --type install "Desktop/Amazon Timestream ODBC Driver-<version>-Darwin.pkg"
```
