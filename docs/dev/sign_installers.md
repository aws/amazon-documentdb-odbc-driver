...
```
  signtool sign /sha1 <CertificateHash> '.\AmazonTimestreamODBC<Bitness>-<version>.msi'
```
...
```
  Set-AuthenticodeSignature '.\AmazonTimestreamODBC<Bitness>-<version>.msi' -Certificate (Get-ChildItem Cert:\CurrentUser\My -CodeSigningCert) -TimestampServer "http://timestamp.verisign.com/scripts/timstamp.dll"
```
...
```
productsign -sign "Developer ID Installer: Your Apple Account Name (**********)" "~/Desktop/AmazonTimestreamODBC-<version>.pkg" "~/Desktop/signed-AmazonTimestreamODBC-<version>.pkg"
```

- Test installer package using [spctl](http://www.manpagez.com/man/8/spctl/)
```
  spctl -a -v --type install "Desktop/AmazonTimestreamODBC-<version>.pkg"
```
